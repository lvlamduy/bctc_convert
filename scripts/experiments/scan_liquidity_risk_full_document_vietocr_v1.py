"""Scan all eight fresh-VietOCR PDFs for liquidity-risk tables."""
...
FORMAT_VERSION = "LIQUIDITY_RISK_8DOCUMENT_FULL_VIETOCR_SCAN_V1"
MATCHER_FORMAT = "LIQUIDITY_RISK_VARIANT_GRAPH_DOCUMENT_V1"
CLAIM_BOUNDARY = (
    "FRESH_VIETOCR_TRANSFORMER_COMPLETE_PDF_BANK_BLIND_LIQUIDITY_RISK_"
    "OPTIONAL_MATURITY_AXIS_ASSET_LIABILITY_GAP_AND_GEOMETRY_SELECTED_"
...
_AUTHORITY = {
    ...
    "currency_interest_rate_and_fair_value_controls_retained": True,
...
class LiquidityRiskFullDocumentScanV1Error(ValueError):
    """The semantic axis or liquidity-risk scan drifted."""


def _error(message: str) -> LiquidityRiskFullDocumentScanV1Error:
    return LiquidityRiskFullDocumentScanV1Error(message)
...
def _load(name: str, filename: str) -> ModuleType:
    ...
    if spec is None or spec.loader is None:
        raise _error(f"cannot load liquidity-risk support: {path}")
...
def _support() -> ModuleType:
    return _load(
        "rotated_scan_support_for_liquidity_risk",
...
def _matcher() -> ModuleType:
    return _load(
        "liquidity_risk_matcher_for_scan",
...
def _validate(value: Any) -> dict[str, Any]:
    if type(value) is not dict or set(value) != _FIELDS:
        raise _error("liquidity-risk scan fields drifted")
    if (
        value["format_version"] != FORMAT_VERSION
        or value["claim_boundary"] != CLAIM_BOUNDARY
        or value["state"] != "FULL_DOCUMENT_LIQUIDITY_RISK_SCAN_COMPLETE"
        or not same_typed_json_v1(value["authority"], _AUTHORITY)
        or type(value["trials"]) is not list
        or len(value["trials"]) != len(EXPECTED_DOCUMENT_ORDER)
        or not same_typed_json_v1(value["metrics"], _metrics(value["trials"]))
    ):
        raise _error("liquidity-risk scan identity drifted")
    for ordinal, (trial, code) in enumerate(
        ...
        if (
            ...
            raise _error("liquidity-risk trial identity drifted")
    material = canonical_clone_v1(value)
    identity = material.pop("scan_id")
    if identity != "lrrfdsv1:scan:" + canonical_json_sha256_v1(material):
        raise _error("liquidity-risk scan ID drifted")
...
def build_liquidity_risk_full_document_scan_v1(semantic_index: Any, rescue: Any) -> dict[str, Any]:
    ...
    for document in axis["documents"]:
        ...
        result = matcher.build_liquidity_risk_variant_graph_document_v1(pages)
        trials.append(
            {
    ...
    return _validate({**material, "scan_id": "lrrfdsv1:scan:" + canonical_json_sha256_v1(material)})
...
def validate_liquidity_risk_full_document_scan_replay_v1(
    ...
    if not same_typed_json_v1(supplied, rebuilt):
        raise _error("liquidity-risk scan does not replay exactly")
    return supplied


def build_live_liquidity_risk_full_document_scan_v1(
...
def validate_live_liquidity_risk_full_document_scan_v1(
...
def main() -> None:
    ...
    sys.stdout.buffer.write(
        canonical_json_bytes_v1(build_live_liquidity_risk_full_document_scan_v1(args.input))
    )
