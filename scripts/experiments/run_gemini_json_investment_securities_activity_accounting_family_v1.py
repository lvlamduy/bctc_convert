#!/usr/bin/env python3
"""Run Family 33 over one authenticated selected-JSON corpus."""
...
from bctc_ai.evaluation.gemini_json_investment_securities_activity_family_v1 import (  # noqa: E402
    FAMILY_ID,
    adapt_gemini_json_investment_securities_activity_indexed_query_evidence_v1,
...
AUDIT_FORMAT_VERSION = (
    "GEMINI_JSON_INVESTMENT_SECURITIES_ACTIVITY_EXPERIMENTAL_AUDIT_V1"
)
PDF_RESIDUAL_FORMAT_VERSION = (
    "INVESTMENT_SECURITIES_ACTIVITY_PDF_RESIDUAL_AUDIT_SPEC_V1"
)
TOPOLOGY_SPEC_PATH = (
    ROOT / "config/families/tm-investment-securities-activity-topology-v1.json"
)
EVALUATION_SPEC_PATH = (
    ROOT / "config/families/tm-investment-securities-activity-evaluation-v1.json"
)
SCHEMA_BINDING_SPEC_PATH = (
    ROOT / "config/families/tm-investment-securities-activity-schema-binding-v1.json"
)
SOURCE_REPAIR_PATH = (
    ROOT
...
ADAPTER_PATH = (
    ROOT
    / "src/bctc_ai/evaluation/gemini_json_investment_securities_activity_family_v1.py"
)
...
class RunGeminiJsonInvestmentSecuritiesActivityV1Error(RuntimeError):
    """The Family-33 run, authentication, evidence, or replay drifted."""


def _error(message: str) -> RunGeminiJsonInvestmentSecuritiesActivityV1Error:
    return RunGeminiJsonInvestmentSecuritiesActivityV1Error(message)
...
def _assert_shared_pins_v1() -> None:
    ...
    if drifted:
        raise _error("Family-33 shared implementation pin drifted: " + ",".join(drifted))


def _assert_current_corpus(index: Mapping[str, Any]) -> None:
    documents = index.get("documents")
    if type(documents) is not list or not documents:
        raise _error("Family-33 conclusion corpus is empty")
    if any(
        "/2025/" not in document.get("relative_path", "")
        and "/2026/" not in document.get("relative_path", "")
        for document in documents
    ):
        raise _error("Family-33 conclusion corpus is outside the current reporting scope")
...
def _validate_pdf_residual_spec_v1(value: Any) -> dict[str, Any]:
    ...
    if (
        ...
        raise _error("Family-33 PDF residual audit spec is invalid")
    prior_ordinal = 0
    for residual in value["residuals"]:
        if (
            ...
            raise _error("Family-33 PDF residual record is invalid or unordered")
        if (
            ...
            raise _error("Family-33 PDF residual status/disposition pairing is invalid")
        page_numbers = []
        page_ids = []
        for page in residual["primary_income_statement_page_axis"]:
            if (
                ...
                raise _error("Family-33 residual PDF page record is invalid")
        ...
        if page_numbers != sorted(set(page_numbers)) or len(page_ids) != len(
            ...
            raise _error("Family-33 residual PDF page axis is not ordered and unique")
        material = {
        ...
        if residual.get("residual_audit_id") != (
            "gjisapdfv1:residual:" + canonical_json_sha256_v1(material)
        ):
            raise _error("Family-33 PDF residual record identity drifted")
...
def _authenticate_pdf_residuals_v1(
    *,
    ...
    if checked["corpus_manifest_index_id"] != index["corpus_manifest_index_id"]:
        raise _error("Family-33 PDF residual audit binds another corpus")
    ...
    if [trial["document_ordinal"] for trial in expected_trials] != [
        ...
        raise _error("Family-33 PDF residual audit does not exhaust every N/U trial")
    document_by_ordinal = {
    ...
    if source_pdf_root.is_symlink() or not root.is_dir():
        raise _error("Family-33 PDF residual source root is unavailable")
    authenticated = []
    for trial, residual in zip(expected_trials, checked["residuals"], strict=True):
        ...
        if (
            ...
            raise _error("Family-33 PDF residual source/trial binding drifted")
        expected_page_axis = []
        for page_json_version_id, page_json in page_json_by_document[
            ...
            if (
                ...
                raise _error("Family-33 residual selected-page binding drifted")
        ...
        if actual_page_axis != expected_page_axis:
            raise _error(
                "Family-33 residual audit does not exhaust the primary income pages"
            )
        ...
        if (
            ...
            raise _error("Family-33 residual source PDF bytes drifted")
        render_axis = []
        with fitz.open(path) as pdf:
            for page in residual["primary_income_statement_page_axis"]:
                if page["physical_page"] > len(pdf):
                    raise _error("Family-33 residual page is outside its source PDF")
                ...
                if digest != page["pdf_page_render_sha256"]:
                    raise _error("Family-33 residual PDF render drifted")
...
def _authenticate_source_repairs_v1(
    *,
    ...
    root = source_pdf_root.resolve()
    if source_pdf_root.is_symlink() or not root.is_dir():
        raise _error("Family-33 source-PDF root is unavailable")
    ...
    for repair in repairs:
        ...
        if (
            ...
            raise _error("Family-33 source-repair document binding drifted")
        ...
        if (
            ...
            raise _error("Family-33 source-repair selected page binding drifted")
        path = (root / document["relative_path"]).resolve()
        if not path.is_relative_to(root) or not path.is_file():
            raise _error("Family-33 source-repair PDF is unavailable")
        ...
        if payload is None:
            ...
            if (
                ...
                raise _error("Family-33 source-repair PDF bytes drifted")
            ...
            try:
                with fitz.open(stream=payload, filetype="pdf") as pdf:
                    ...
                    if not 0 <= page_index < len(pdf):
                        raise _error("Family-33 source-repair page is outside its PDF")
                ...
                raise _error("Family-33 source-repair PDF cannot be rendered") from exc
            rendered = {
        ...
        if not same_typed_json_v1(rendered, repair["page_image"]):
            raise _error("Family-33 source-repair PDF render drifted")
        material = {
        ...
        checked.append(
            {
                **material,
                "authentication_id": "gjisarv1:auth:"
...
def replay_investment_securities_activity_trials_from_source_v1(
    *,
    ...
    if not same_typed_json_v1(generic_compiled, compiled_specs):
        raise _error("Family-33 source replay declarative specs drifted")
    family_compiled = compile_gemini_json_investment_securities_activity_family_specs_v1(
    ...
    if not same_typed_json_v1(indexed, indexed_query_evidence):
        raise _error("Family-33 source replay rebuilt different query evidence")
    ...
    *,
    ...
    for trial in trials:
        ...
        if type(adapter) is not dict:
            raise _error("Family-33 candidate adapter receipt is absent")
    ...
    *,
    ...
    for reference_index, (reference, oracle) in enumerate(
        ...
        if type(oracle_trials) is not list or not oracle_trials:
            raise _error("Family-33 historical oracle trial denominator drifted")
        ...
        for trial in oracle_trials:
            ...
            if type(source_sha256) is not str:
                raise _error("Family-33 historical oracle source identity is absent")
            rows.append(
    ...
    if (
        ...
        raise _error("Family-33 historical corpus is not disjoint")
...
def _build_audit_v1(
    *,
    ...
    if not (expected_repairs == applied_repairs == authenticated_repairs):
        raise _error("Family-33 repair application/authentication axis drifted")
...
def _validate_audit_v1(value: Any) -> dict[str, Any]:
    if (
        ...
        raise _error("Family-33 audit content is invalid")
    material = {key: canonical_clone_v1(item) for key, item in value.items() if key != "audit_id"}
    if value["audit_id"] != "gjisaauditv1:audit:" + canonical_json_sha256_v1(material):
        raise _error("Family-33 audit identity drifted")
    return canonical_clone_v1(value)
...
def _implementation_refs(pdf_residual_audit_spec: Path) -> list[dict[str, Any]]:
    paths = (
        ROOT
        / "scripts/experiments/run_gemini_json_investment_securities_activity_accounting_family_v1.py",
        ADAPTER_PATH,
        SHARED_EVALUATOR_PATH,
        SHARED_RUNNER_PATH,
...
def _run_with_database(
    ...
    *,
    ...
    source_authentications = _authenticate_source_repairs_v1(
        repairs=compiled["investment_securities_activity_source_repair_overlay"]["repairs"],
        index=index,
        selected_page_axis=base["selected_page_axis"],
    ...
    replayed = replay_investment_securities_activity_trials_from_source_v1(
    ...
    if not same_typed_json_v1(replayed, trials):
        raise _error("Family-33 direct source replay returned different trials")
    ...
    if sweep["metrics"]["document_count"] != len(index["documents"]):
        raise _error("Family-33 sweep denominator drifted")
    ...
    stored = ingest_gemini_accounting_family_sweep_v1(
        ...
        source_replay_adapter=replay_investment_securities_activity_trials_from_source_v1,
    ...
    if not same_typed_json_v1(stored_sweep, sweep):
        raise _error("stored Family-33 sweep differs from authenticated evaluation")
