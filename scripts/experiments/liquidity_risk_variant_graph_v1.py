"""Bank-blind complete-PDF variant graph for liquidity-risk tables."""
...
FORMAT_VERSION = "LIQUIDITY_RISK_VARIANT_GRAPH_DOCUMENT_V1"
FAMILY_ID = "LIQUIDITY_RISK"
CLAIM_BOUNDARY = (
    "BANK_BLIND_COMPLETE_PDF_FRESH_OR_GEOMETRY_SELECTED_ROTATED_VIETOCR_"
    "PAIR_FIRST_LIQUIDITY_RISK_OWNER_OPTIONAL_MATURITY_AXES_ASSET_"
...
_SAFETY = {
    ...
    "maturity_axis_order_required_for_matching": False,
    "currency_interest_rate_or_fair_value_table_can_accept": False,
...
_ASSET_ROLES = {
    ...
    "ASSET_CUSTOMER_LOANS_AND_PURCHASED_DEBT",
    "ASSET_DERIVATIVE",
    "ASSET_FIXED_ASSETS",
    "ASSET_FIXED_PROPERTY_COMBINED",
    "ASSET_INTERBANK",
    "ASSET_INVESTMENT_PROPERTY",
...
_LIABILITY_ROLES = {
    ...
    "LIABILITY_GOVERNMENT_CENTRAL_BANK",
    "LIABILITY_INTERBANK",
    "LIABILITY_ISSUED_PAPERS",
    "LIABILITY_OTHER",
}
_GAP_ROLES = {"NET_LIQUIDITY_GAP"}


class LiquidityRiskVariantGraphV1Error(ValueError):
    """The complete-PDF input or liquidity-risk graph drifted."""


def _error(message: str) -> LiquidityRiskVariantGraphV1Error:
    return LiquidityRiskVariantGraphV1Error(message)


def _support() -> ModuleType:
    name = "interest_income_graph_support_for_liquidity_risk"
...
def _owner(text: str) -> bool:
    ...
    return len(value.split()) <= 18 and (
        "rui ro thanh khoan" in value or "bang rui ro thanh khoan" in value
    )
...
def _negative_family(text: str) -> str | None:
    ...
    if "rui ro lai suat" in value or "dinh gia lai" in value:
...
def _unit(text: str) -> bool:
    value = _strip(text)
    return ("trieu dong" in value or "trieu vnd" in value) and (
        "don vi" in value or "dom vi" in value or value in {"trieu dong", "trieu vnd"}
    )


def _raw_role(text: str) -> str | None:
    value = _strip(text)
    if "chenh" in value and "thanh khoan" in value and "rong" in value:
    ...
    if "cho vay khach hang" in value and "mua no" in value:
        return "ASSET_CUSTOMER_LOANS_AND_PURCHASED_DEBT"
    ...
    if "tai san co dinh" in value and "bat dong san dau tu" in value:
        return "ASSET_FIXED_PROPERTY_COMBINED"
    if "tai san co dinh" in value:
        return "ASSET_FIXED_ASSETS"
    if "bat dong san dau tu" in value:
    ...
    if "cac khoan no chinh phu" in value or "vay tu nhnn" in value:
        return "LIABILITY_GOVERNMENT_CENTRAL_BANK"
    if (
        "tctd" in value
        and "gui" in value
        and ("vay" in value or "cho vay" in value or "cap tin dung" in value)
    ):
...
def _joined_roles(page: Mapping[str, Any]) -> tuple[list[str], list[dict[str, Any]]]:
    ...
    for index, line in enumerate(label_lines):
        ...
        if role == "ASSET_TOTAL":
            ...
            role = "LIABILITY_INTERBANK" if after_asset_total else "ASSET_INTERBANK"
...
def _axis_role(value: str) -> str | None:
    ...
    if re.search(r"\b(?:den|duoi)\s+0?1(?:\s+thang)?\b", value) or re.search(
        ...
        return "WITHIN_LE1M"
    if re.search(
        r"\btu\s+(?:(?:tren|tien)\s+)?0?1(?:\s+nam)?\s*(?:den\s*)?0?5\s+nam\b",
        value,
    ):
        return "WITHIN_1_5Y"
    if re.search(r"\btu\s+(?:tren\s+)?0?1\s*(?:den\s*)?0?3(?:\s+thang)?\b", value):
        return "WITHIN_1_3M"
    if re.search(r"\btu\s+(?:tren\s+)?0?3\s*(?:-|den|\?)?\s*12", value):
        return "WITHIN_3_12M"
    if re.search(r"\btren\s+0?5\s+nam\b", value) or "tien s nam" in value:
        return "WITHIN_GT5Y"
...
def _header_features(page: Mapping[str, Any]) -> tuple[list[str], list[dict[str, Any]], int]:
    ...
    for index, line in enumerate(header):
        if _owner(line["normalized_text"]) or _negative_family(line["normalized_text"]):
            continue
        ...
        if role is None:
            role = next(
                (found for phrase in phrases[1:] if (found := _axis_role(phrase))),
                None,
            )
        if role is not None:
    ...
    events = [support._line_ref(latest[role], f"MATURITY_AXIS_{role}") for role in axes]
...
def _table_features(page: Mapping[str, Any]) -> dict[str, Any]:
    ...
    complete = (
        "TOTAL" in axes
        and bool({"OVERDUE", "OVERDUE_GT3M", "OVERDUE_LE3M"} & set(axes))
        and len(set(axes) & term_axes) >= 4
        and len(axes) >= 7
        and unit_count >= 1
        and asset_count >= 5
        ...
        and "NET_LIQUIDITY_GAP" in observed
    ...
    return {
        ...
        "maturity_axes": axes,
        "gap_roles": [role for role in roles if role in _GAP_ROLES],
...
def _region(
    ...
    axes = list(
        dict.fromkeys(axis for *_, features in members for axis in features["maturity_axes"])
    )
    roles = list(
        dict.fromkeys(
    ...
    anchors = ["FAMILY_OWNER", *[f"MATURITY_AXIS_{axis}" for axis in axes], *roles]
    ...
    return {
        ...
        "layout": {
            ...
            "observed_source_roles": roles,
            "optional_maturity_axes_allowed": True,
            "period_table_count": len(members),
            "maturity_axes_observed": axes,
            "row_and_axis_order_is_semantic": False,
            "gap_roles_observed": [role for role in roles if role in _GAP_ROLES],
...
def _candidates(
    ...
    for index, page in enumerate(pages):
        ...
        if not features["complete"]:
            if owner is not None or len(features["maturity_axes"]) >= 3:
                near.append(
                    {
                        "negative_families": features["negative_families"],
                        "page_span": [page["page_sequence"], page["page_sequence"]],
                        "reason": (
                            "LIQUIDITY_RISK_OWNER_WITHOUT_COMPLETE_TABLE"
                            if owner is not None
                            else "MATURITY_LIKE_AXES_WITHOUT_BOUND_FAMILY_OWNER_OR_CORE"
                        ),
                        "maturity_axes_observed": features["maturity_axes"],
                    }
                )
            continue
        if owner is None:
            near.append(
                {
                    "negative_families": features["negative_families"],
                    "page_span": [page["page_sequence"], page["page_sequence"]],
                    "reason": "COMPLETE_MATURITY_LIKE_TABLE_WITHOUT_BOUND_FAMILY_OWNER",
                    "maturity_axes_observed": features["maturity_axes"],
                }
            )
            continue
...
def _validate(value: Any) -> dict[str, Any]:
    if type(value) is not dict or set(value) != _FIELDS:
        raise _error("liquidity-risk graph fields drifted")
    ...
    if (
        ...
        or value["status"] != "LIQUIDITY_RISK_GRAPH_ENUMERATION_COMPLETE"
        ...
        raise _error("liquidity-risk graph identity drifted")
    expected_uniqueness = {
        "complete_region_count": len(regions),
    ...
    if not same_typed_json_v1(value["uniqueness"], expected_uniqueness):
        raise _error("liquidity-risk uniqueness drifted")
    material = canonical_clone_v1(value)
    identity = material.pop("result_id")
    if identity != "lrrvgv1:graph:" + canonical_json_sha256_v1(material):
        raise _error("liquidity-risk graph ID drifted")
    return canonical_clone_v1(value)


def build_liquidity_risk_variant_graph_document_v1(pages: Any) -> dict[str, Any]:
    ...
    material = {
        ...
        "status": "LIQUIDITY_RISK_GRAPH_ENUMERATION_COMPLETE",
        "uniqueness": {
            "complete_region_count": len(regions),
            "status": "UNIQUE_FULL_MATCH" if len(regions) == 1 else "NOT_UNIQUE_FULL_MATCH",
        },
    ...
    return _validate(
        {**material, "result_id": "lrrvgv1:graph:" + canonical_json_sha256_v1(material)}
    )


def validate_liquidity_risk_variant_graph_document_v1(value: Any) -> dict[str, Any]:
