#!/usr/bin/env python3
"""Run Family48 currency risk over the authenticated 140-document corpus."""
...
class RunGeminiJsonCurrencyRiskFamilyV1Error(RuntimeError):
    """The Family48 source, graph, comparator, or persistence boundary drifted."""


def _error(message: str) -> RunGeminiJsonCurrencyRiskFamilyV1Error:
    return RunGeminiJsonCurrencyRiskFamilyV1Error(message)


AUDIT_FORMAT_VERSION = "GEMINI_JSON_CURRENCY_RISK_EXPERIMENTAL_AUDIT_V1"
...
PINNED_HISTORICAL_ORACLES = [
    {
        ...
        "sha256": "ec209921d958cd248dfb4c92767a0347931d085e648c67c879fe72f9de9b488b",
        "size_bytes": 197166,
    },
    {
        ...
        "sha256": "422c0a86112833ff8205522d6eb302857eb6e256b829a2fcde43cc9eb4697ba6",
        "size_bytes": 382418,
    },
]
_HISTORICAL_EQUATION_RESULT_ROLE = {
    "ASSET_TOTAL_MINUS_LIABILITY_TOTAL_EQUALS_INTERNAL_STATE": "STATE_INTERNAL",
    "INTERNAL_STATE_PLUS_EXTERNAL_STATE_EQUALS_COMBINED_STATE": "STATE_COMBINED",
}
...
def _historical_oracles() -> list[tuple[dict[str, Any], dict[str, Any]]]:
    ...
    for reference in PINNED_HISTORICAL_ORACLES:
        ...
        if (
            ...
            raise _error("pinned currency-risk historical oracle drifted")
...
def _candidate_by_source(trials: Sequence[Mapping[str, Any]]) -> dict[str, Mapping[str, Any]]:
    ...
    for trial in trials:
        ...
        if type(source_sha256) is not str or source_sha256 in result:
            raise _error("currency-risk current source axis is ambiguous")
    ...
    for receipt in closure["table_receipts"]:
        ...
        for resolved in receipt["resolved_columns"]:
            ...
            for row_role, cell in resolved["core_cells_by_role"].items():
                ...
                if key in result:
                    raise _error("currency-risk current source cell axis is duplicate")
...
def _current_equation_axis(candidate: Mapping[str, Any]) -> dict[tuple[str, str, str, str], dict]:
    ...
    for equation in candidate["closure_receipt"]["equations"]:
        ...
        if key in result:
            raise _error("currency-risk current equation axis is duplicate")
...
def _historical_value_axis(values: Any) -> list[dict[str, Any]]:
    if type(values) is not list or not values:
        raise _error("historical currency-risk value axis is absent")
    result = []
    for value in values:
        ...
        if period_axis not in {"CURRENT", "COMPARATIVE"} or type(coefficient) is not int:
            raise _error("historical currency-risk value is invalid")
        if type(date) is not str:
            raise _error("historical currency-risk period date is absent")
        result.append(
            {
...
def _historical_comparator_axes(
    ...
    for oracle_ref, oracle in _historical_oracles():
        ...
        for historical in oracle["trials"]:
            ...
            if current is None:
                raise _error("historical currency-risk source does not join one current trial")
            ...
            if current_candidate is None:
                if legacy_present:
                    raise _error("historical mapped currency-risk source is unresolved")
                continue
            ...
            if len(mapping_by_report_norm_id) != len(current_candidate["mappings"]):
                raise _error("current currency-risk report norm axis is duplicate")
            ...
            for item in historical.get("verified_accounting_equations", []):
                ...
                current_currency_role = _SOURCE_ONLY_CURRENCY_ROLE.get(
                    item.get("axis_role"), item.get("axis_role")
                )
                key = (
            ...
            for item in historical.get("verified_source_only_rows", []):
                historical_role = item.get("axis_role")
                current_currency_role = _SOURCE_ONLY_CURRENCY_ROLE.get(
                ...
                residual_expected = item.get("reason") == "SOURCE_PRESENTATION_ARITHMETIC_RESIDUAL"
...
def build_currency_risk_experimental_audit_v1(
...
def validate_currency_risk_experimental_audit_content_v1(value: Any) -> dict[str, Any]:
    axis_names = {
        "clusters",
        "equations",
    ...
    if (
        ...
        raise _error("currency-risk experimental audit shape drifted")
    if value["axis_counts"] != {name: len(value[name]) for name in axis_names} or value[
        ...
        raise _error("currency-risk experimental audit axis seal drifted")
    if value.get("audit_metrics") != _audit_metrics(value):
        raise _error("currency-risk experimental audit metrics drifted")
    material = {key: value[key] for key in fields - {"audit_id"}}
    if value["audit_id"] != "gjfcreav1:audit:" + canonical_json_sha256_v1(material):
        raise _error("currency-risk experimental audit identity drifted")
    return canonical_clone_v1(value)


def validate_currency_risk_experimental_audit_replay_v1(
    value: Any,
    *,
    ...
    if not same_typed_json_v1(embedded, compiled_specs):
        raise _error("currency-risk caller and embedded compiled specs differ")
    validate_selected_equity_matrix_family_candidate_replays_v1(
        source_database,
    ...
    expected = build_currency_risk_experimental_audit_v1(sweep=sweep, compiled_specs=embedded)
    validate_currency_risk_experimental_audit_content_v1(value)
    if not same_typed_json_v1(value, expected):
        raise _error("currency-risk experimental audit does not replay exactly")
    return expected


def build_currency_risk_experimental_bundle_v1(
    *,
    ...
    if compiled.get("family_id") != "CURRENCY_RISK":
        raise _error("Family48 runner received a different family triplet")
    ...
    validate_gemini_json_flat_family_sweep_v1(sweep)
    audit = build_currency_risk_experimental_audit_v1(sweep=sweep, compiled_specs=compiled)
    validate_currency_risk_experimental_audit_replay_v1(
        audit,
        sweep=sweep,
        compiled_specs=compiled,
...
def _assert_release_pins(
    *, sweep: Mapping[str, Any], audit: Mapping[str, Any], selected_ids: Sequence[str]
    ...
    if (
        ...
        raise _error("Family48 frozen corpus release pins drifted")
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    with _authenticated_sqlite_snapshot(
        ...
        sweep, audit, _compiled = build_currency_risk_experimental_bundle_v1(
        ...
        implementation_paths = (
            ROOT / "scripts/experiments/run_gemini_json_currency_risk_family_v1.py",
            ROOT / "src/bctc_ai/evaluation/gemini_json_currency_risk_matrix_v1.py",
        ...
        if not same_typed_json_v1(stored_sweep, sweep):
            raise _error("stored Family48 sweep differs from authenticated evaluation")
