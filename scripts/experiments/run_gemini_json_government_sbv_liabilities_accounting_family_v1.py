#!/usr/bin/env python3
"""Run Government/SBV liabilities over one authenticated selected-JSON corpus."""
...
from bctc_ai.evaluation.gemini_json_government_sbv_liabilities_family_v1 import (  # noqa: E402
    ...
    compile_gemini_json_government_sbv_liabilities_family_specs_v1,
...
class RunGeminiJsonGovernmentSbvLiabilitiesAccountingFamilyV1Error(RuntimeError):
    """The authenticated corpus, policy, result, or replay boundary drifted."""


def _error(message: str) -> RunGeminiJsonGovernmentSbvLiabilitiesAccountingFamilyV1Error:
    return RunGeminiJsonGovernmentSbvLiabilitiesAccountingFamilyV1Error(message)


AUDIT_FORMAT_VERSION = "GEMINI_JSON_GOVERNMENT_SBV_LIABILITIES_EXPERIMENTAL_AUDIT_V1"
FAMILY_ID = "GOVERNMENT_SBV_LIABILITIES"
PINNED_HISTORICAL_ORACLES = generic.PINNED_GOVERNMENT_SBV_LIABILITIES_HISTORICAL_ORACLES
    ...
    *, repairs: list[dict[str, Any]], source_pdf_root: Path
    ...
    for repair in repairs:
        ...
        if not path.is_relative_to(root) or not path.is_file():
            raise _error("Government/SBV repair source path is unavailable")
        source_key = (
            logical_name,
        ...
        if payload is None:
            ...
            if (
                ...
                raise _error("Government/SBV repair source artifact drifted")
        ...
        if cached is None:
            with fitz.open(stream=payload, filetype="pdf") as document:
                if locator["physical_page"] > len(document):
                    raise _error("Government/SBV repair physical page is outside its PDF")
        ...
        if actual_render != expected_render or actual_crop != repair["crop_evidence"]:
            raise _error("Government/SBV repair render or crop evidence drifted")
        checked.append(canonical_clone_v1(repair))
    return checked


def replay_government_sbv_liabilities_trials_from_source_v1(
    *,
    ...
    """Re-query, adapt, evaluate, and replay the complete Family-23 trial axis."""

    source_repairs = generic._json(SOURCE_REPAIR_SPEC_PATH)
    family_compiled = bind_gemini_json_government_sbv_liabilities_source_repairs_v1(
        compiled_specs,
        source_repairs,
    )
    ...
    if not same_typed_json_v1(adapted_indexed, indexed_query_evidence):
        raise _error(
            "Government/SBV source replay rebuilt different indexed query evidence"
        )
    ...
    for cluster in adapted_indexed["accepted_clusters"]:
        ...
        candidate = evaluate_gemini_json_government_sbv_liabilities_family_cluster_v1(
            regions=regions,
    ...
    *, compiled_specs: dict[str, Any]
    ...
    for oracle_ref_index, (reference, oracle) in enumerate(
        ...
        if type(trials) is not list or not trials:
            raise _error("Government/SBV historical oracle trial axis is absent")
        refs.append({**reference, "expected_trial_count": len(trials)})
        for trial in trials:
            if type(trial) is not dict:
                raise _error("Government/SBV historical oracle trial is invalid")
            source_sha256 = trial.get("source_pdf_sha256")
            if type(source_sha256) is not str:
                raise _error("Government/SBV historical oracle source identity is absent")
            rows.append(
                {
        ...
        for mapping in candidate["mappings"]:
            ...
            if decimal_cells:
                if len(decimal_cells) != len(mapping["values"]):
                    raise _error("Government/SBV decimal mapping axis is incomplete")
                record.update(
                    {
    ...
    *,
    ...
    if policy == STRICT_RELEASE:
        ...
        if legacy_refs != [
            ...
            raise _error("Government/SBV historical oracle references disagree")
        for record in comparator:
            source_sha256 = record.get("source_sha256")
            if type(source_sha256) is not str:
                raise _error("Government/SBV historical comparator source is absent")
        ...
        if not comparator or any(record.get("disposition") != "EXACT" for record in comparator):
            raise _error("Government/SBV strict historical comparison is not exact")
    ...
    if len(candidate_sources) != len(replay_sources) or set(candidate_sources) != set(
        ...
        raise _error("Government/SBV indexed candidate/replay axes drifted")
...
def build_government_sbv_liabilities_experimental_audit_v1(
    *,
    sweep: dict[str, Any],
    sweep_output: Path,
    ...
    if sweep.get("corpus_manifest_index_id") != index.get("corpus_manifest_index_id"):
        raise _error("Government/SBV sweep/current manifest identity drifted")
    ...
    return {
        **material,
        "audit_id": "gjgslfeav1:audit:" + canonical_json_sha256_v1(material),
    }


def validate_government_sbv_liabilities_experimental_audit_content_v1(
    value: Any,
) -> dict[str, Any]:
    fields = {
        "audit_id",
    ...
    if (
        ...
        raise _error("Government/SBV experimental audit shape drifted")
    ...
    if value.get("axis_counts") != counts or value.get("axis_sha256") != hashes:
        raise _error("Government/SBV experimental audit axis seal drifted")
    ...
    if (
        ...
        raise _error("Government/SBV historical comparator policy receipt drifted")
    if policy == STRICT_RELEASE:
        if (
            disposition != EXACT_HISTORICAL_COMPARISON
            or not comparator
            ...
            raise _error("Government/SBV strict comparator audit drifted")
    elif (
        ...
        raise _error("Government/SBV disjoint comparator audit drifted")
    material = {key: value[key] for key in fields - {"audit_id"}}
    if value.get("audit_id") != "gjgslfeav1:audit:" + canonical_json_sha256_v1(material):
        raise _error("Government/SBV experimental audit identity drifted")
...
def _run_with_authenticated_database(
    ...
    *,
    ...
    for cluster in indexed["accepted_clusters"]:
        ...
        candidates_by_ordinal[cluster["document_ordinal"]] = (
            evaluate_gemini_json_government_sbv_liabilities_family_cluster_v1(
                ...
                compiled_specs=compiled,
                query_receipt=build_gemini_json_government_sbv_liabilities_region_query_receipt_v1(
                    regions
                ),
            )
        )
    ...
    if not same_typed_json_v1(replayed_trials, trials):
        raise _error("Government/SBV SQLite candidate replay returned a different trial axis")
    audit = build_government_sbv_liabilities_experimental_audit_v1(
        sweep=sweep,
    ...
    validate_government_sbv_liabilities_experimental_audit_content_v1(audit)
    ...
    implementation_paths = (
        ROOT
        / "scripts/experiments/run_gemini_json_government_sbv_liabilities_accounting_family_v1.py",
        ROOT
        ...
        ROOT
        / "src/bctc_ai/evaluation/gemini_json_government_sbv_liabilities_family_v1.py",
    ...
    stored = ingest_gemini_accounting_family_sweep_v1(
        ...
        source_replay_adapter=replay_government_sbv_liabilities_trials_from_source_v1,
    ...
    if not same_typed_json_v1(stored_sweep, sweep):
        raise _error("stored Government/SBV sweep differs from authenticated evaluation")
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    if args.run_kind == "OFFICIAL" and args.historical_comparator_policy != STRICT_RELEASE:
        raise _error("OFFICIAL Government/SBV run requires STRICT_RELEASE policy")
    if args.historical_comparator_policy == DISJOINT_EXPANSION and args.run_kind != "EXPERIMENTAL":
        raise _error("Government/SBV expansion run must be EXPERIMENTAL")
    ...
    compiled = compile_gemini_json_government_sbv_liabilities_family_specs_v1(
        topology, evaluation, schema, source_repairs
    )
    if compiled["topology"]["family_id"] != FAMILY_ID:
        raise _error("runner received a non-Government/SBV family")
    spec_refs = {
    ...
    source_repair_authentication = _authenticate_source_repairs_v1(
        repairs=compiled["government_sbv_liabilities_source_repairs"],
