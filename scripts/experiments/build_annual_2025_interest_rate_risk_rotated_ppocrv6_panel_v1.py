"""Build the annual interest-rate-risk rotated full-page PP-OCRv6 panel.

The complete-PDF family graph selects table regions and the existing geometry
lane selects landscape pages.  Their intersection is reread in canonical
upright coordinates by PP-OCRv6 as an independent numeric challenger.  Bank
identity, page number and note number remain provenance only.
"""
...
PROJECTION_FORMAT = "ANNUAL_2025_INTEREST_RATE_RISK_ROTATED_PPOCRV6_VERIFIED_PROJECTION_V1"
SELECTION_RULE = "UNIQUE_COMPLETE_INTEREST_RATE_RISK_REGION_AND_GEOMETRY_ROTATED_SOURCE_AXIS_TRUE"
PANEL_ID_PREFIX = "a2025irrrpv1:panel:"
PROJECTION_ID_PREFIX = "a2025irrrpv1:projection:"
_AUTHORITY = {
...
def _load_module(name: str, relative_path: str) -> ModuleType:
    ...
    if spec is None or spec.loader is None:
        raise RuntimeError(f"cannot load annual interest-rate-risk rotated support: {path}")
...
_CORE = _load_module(
    "annual_2025_interest_rate_risk_rotated_ppocrv6_core_v1",
    "scripts/experiments/build_annual_2025_tangible_fixed_assets_rotated_ppocrv6_panel_v1.py",
)
Annual2025InterestRateRiskRotatedPPocrV6PanelError = (
    _CORE.Annual2025TangibleRotatedPPocrV6PanelError
)
...
def _interest_rate_risk_selection() -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    for trial in scan["trials"]:
        ...
        if (
            ...
            raise Annual2025InterestRateRiskRotatedPPocrV6PanelError(
                "one annual document lost its unique interest-rate-risk region"
            )
...
def _configured_core() -> ModuleType:
    ...
    _CORE._live_selection = _interest_rate_risk_selection
    return _CORE


def build_annual_2025_interest_rate_risk_rotated_ppocrv6_panel_v1() -> dict[str, Any]:
    """Materialize graph-selected normalized interest-rate-risk pages."""

    return _configured_core().build_annual_2025_tangible_rotated_ppocrv6_panel_v1()


def read_verified_annual_2025_interest_rate_risk_rotated_ppocrv6_panel_v1() -> dict[str, Any]:
...
def main() -> int:
    ...
    value = (
        build_annual_2025_interest_rate_risk_rotated_ppocrv6_panel_v1()
        if args.build
        else read_verified_annual_2025_interest_rate_risk_rotated_ppocrv6_panel_v1()
    )
