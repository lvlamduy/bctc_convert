#!/usr/bin/env python3
"""Run Family 37 over one authenticated current selected-JSON corpus."""
...
from bctc_ai.evaluation.gemini_json_credit_risk_provision_expense_family_v1 import (  # noqa: E402
    FAMILY_ID,
...
AUDIT_FORMAT_VERSION = (
    "GEMINI_JSON_CREDIT_RISK_PROVISION_EXPENSE_EXPERIMENTAL_AUDIT_V1"
)
PDF_RESIDUAL_FORMAT_VERSION = (
    "CREDIT_RISK_PROVISION_EXPENSE_PDF_RESIDUAL_AUDIT_SPEC_V1"
)
PDF_VISIBLE_SOURCE_ROW_AUDIT_FORMAT_VERSION = (
    "CREDIT_RISK_PROVISION_EXPENSE_PDF_VISIBLE_SOURCE_ROW_RENDER_AUDIT_V2"
)
PDF_RENDER_CONTRACT = {
    "alpha": False,
    "colorspace": "RGB",
...
FAMILY_OWNERSHIP_DISPOSITION = (
    "FAMILY_LOCAL_SCHEMA_AXIS_SHARED_EVALUATOR_AND_RUNNER_READ_ONLY"
)
TOPOLOGY_SPEC_PATH = (
    ROOT
    / "config/families/tm-credit-risk-provision-expense-topology-v1.json"
)
EVALUATION_SPEC_PATH = (
    ROOT
    / "config/families/tm-credit-risk-provision-expense-evaluation-v1.json"
)
SCHEMA_BINDING_SPEC_PATH = (
    ROOT
    / "config/families/tm-credit-risk-provision-expense-schema-binding-v1.json"
)
SOURCE_REPAIR_SPEC_PATH = (
    ROOT
    / "config/families/tm-credit-risk-provision-expense-source-repair-v1.json"
)
...
PINNED_HISTORICAL_ORACLES = (
    generic.PINNED_CREDIT_RISK_PROVISION_EXPENSE_HISTORICAL_ORACLES
)


class RunGeminiJsonCreditRiskProvisionExpenseV1Error(RuntimeError):
    """The Family-37 run, evidence, or ownership boundary drifted."""


def _error(
    message: str,
) -> RunGeminiJsonCreditRiskProvisionExpenseV1Error:
    return RunGeminiJsonCreditRiskProvisionExpenseV1Error(message)
...
def _assert_shared_implementation_pins_v1() -> None:
    ...
    if drifted:
        raise _error("Family-37 shared implementation pin drifted: " + ",".join(drifted))
...
def _source_path(root: Path, relative_path: str) -> Path:
    ...
    if not path.is_relative_to(resolved_root) or path.is_symlink() or not path.is_file():
        raise _error("Family-37 source PDF path is unavailable or escapes its root")
    return path
...
def _assert_current_corpus(index: dict[str, Any]) -> None:
    ...
    if type(documents) is not list or not documents:
        raise _error("Family-37 conclusion corpus is empty")
    if any(
        "/2025/" not in document.get("relative_path", "")
        and "/2026/" not in document.get("relative_path", "")
        for document in documents
    ):
        raise _error("Family-37 conclusion corpus includes a pre-2025 document")
...
def _expected_pdf_visible_audit_rows_v1(
    ...
    for item in movement_cells:
        ...
        if locator in covered_locators or locator in seen_movement:
            continue
        rows.append(
            _pdf_visible_audit_row_v1(
                item, axis_kind="CUSTOMER_MOVEMENT_SOURCE_ROW"
            )
        )
        seen_movement.add(locator)
    ...
    *,
    ...
    for reference_index, reference in enumerate(PINNED_HISTORICAL_ORACLES):
        ...
        if (
            ...
            raise _error("Family-37 historical oracle bytes drifted")
        artifact = generic._json(path)
        oracle_trials = artifact.get("trials")
        if type(oracle_trials) is not list or not oracle_trials:
            raise _error("Family-37 historical oracle trial denominator drifted")
        ...
        for trial in oracle_trials:
            ...
            if type(source_sha256) is not str:
                raise _error("Family-37 historical oracle source identity is absent")
            rows.append(
                {
...
def _validate_pdf_residual_spec_v1(value: Any) -> dict[str, Any]:
    ...
    if (
        ...
        raise _error("Family-37 PDF residual audit spec is invalid")
    prior = 0
    for residual in value["residuals"]:
        ...
        if (
            ...
            raise _error("Family-37 PDF residual record is invalid or unordered")
        material = {
        ...
        if residual.get("residual_audit_id") != (
            ...
            raise _error("Family-37 PDF residual record identity drifted")
...
def _authenticate_pdf_residuals_v1(
    *,
    ...
    if checked["corpus_manifest_index_id"] != index["corpus_manifest_index_id"]:
        raise _error("Family-37 PDF residual audit binds another corpus")
    expected = {
        trial["source_sha256"]
        for trial in trials
    ...
    if actual != expected:
        raise _error("Family-37 PDF residual audit does not exhaust every N/U trial")
    ...
    for residual in checked["residuals"]:
        ...
        if (
            ...
            raise _error("Family-37 PDF residual source/trial binding drifted")
        ...
        if path.stat().st_size != document["source_size_bytes"] or _sha256(path) != document[
            ...
            raise _error("Family-37 PDF residual source bytes drifted")
        rendered = []
        with fitz.open(path) as pdf:
            if len(residual["pdf_page_axis"]) != len(pdf):
                raise _error("Family-37 residual does not cover every PDF page")
            for ordinal, page in enumerate(residual["pdf_page_axis"], start=1):
                if (
                    ...
                    raise _error("Family-37 residual PDF page axis drifted")
                ...
                if digest != page.get("pdf_page_render_sha256"):
                    raise _error("Family-37 residual PDF render drifted")
...
def _authenticate_source_repairs_v1(
    *,
    ...
    for repair in repairs:
        document = documents.get(repair["source_sha256"])
        locator = repair["locator"]
        if document is None:
            out_of_corpus_axis.append(
                {
                    "disposition": (
        ...
        if (
            page_axis is None
            or page_axis.get("source_sha256") != repair["source_sha256"]
            or page_axis.get("physical_page") != locator["physical_page"]
        ):
            raise _error("Family-37 source-repair corpus binding drifted")
        path = _source_path(source_pdf_root, document["relative_path"])
        if repair["source_sha256"] not in source_digests:
            source_digests[repair["source_sha256"]] = _sha256(path)
        actual_source_sha256 = source_digests[repair["source_sha256"]]
        if (
            actual_source_sha256 != repair["source_sha256"]
            or path.stat().st_size != document["source_size_bytes"]
        ):
            raise _error("Family-37 source-repair PDF bytes drifted")
        with fitz.open(path) as pdf:
            if locator["physical_page"] > len(pdf):
                raise _error("Family-37 source-repair page is outside PDF")
            pixmap = pdf[locator["physical_page"] - 1].get_pixmap(
                matrix=fitz.Matrix(2, 2), colorspace=fitz.csRGB, alpha=False
            )
            render_sha256 = sha256(pixmap.tobytes("png")).hexdigest()
        if render_sha256 != repair["pdf_page_render_sha256"]:
            raise _error("Family-37 source-repair PDF render drifted")
        axis.append(
            {
                "locator": canonical_clone_v1(locator),
    ...
    axis.sort(key=canonical_json_sha256_v1)
    out_of_corpus_axis.sort(key=canonical_json_sha256_v1)
    material = {
        "applicable_repair_count": len(axis),
        "authenticated": True,
        "declared_repair_count": len(repairs),
        "out_of_corpus_repair_axis": out_of_corpus_axis,
        "out_of_corpus_repair_axis_sha256": canonical_json_sha256_v1(
            out_of_corpus_axis
        ),
        "out_of_corpus_repair_count": len(out_of_corpus_axis),
        "repair_axis": axis,
        "repair_axis_sha256": canonical_json_sha256_v1(axis),
        "repair_count": len(axis),
        "source_pdf_count": len(source_digests),
        "unverified_repair_count": 0,
    }
    return {
        **material,
        "receipt_id": "gjcrpefav1:source-repair-auth:"
    ...
    *,
    ...
    for item in manual_axis:
        ...
        if (
            ...
            raise _error("Family-37 manual PDF review binding drifted")
        review_kinds.add(item["review_kind"])
    if review_kinds != required_review_kinds:
        raise _error("Family-37 manual PDF review category frontier is incomplete")
...
def _authenticate_pdf_visible_source_row_audit_v1(
    *,
    ...
    if (
        ...
        raise _error("Family-37 PDF-visible source-row audit envelope drifted")
    material = {
        key: canonical_clone_v1(item)
    ...
    if value.get("audit_id") != (
        "gjcrpefav1:pdf-visible:" + canonical_json_sha256_v1(material)
    ):
        raise _error("Family-37 PDF-visible source-row audit identity drifted")
    ...
    if (
        ...
        raise _error("Family-37 PDF-visible source PDF axis drifted")
    ...
    if (
        ...
        raise _error("Family-37 PDF-visible page-render axis drifted")
    documents = {item["source_ordinal"]: item for item in index["documents"]}
    actual_keys = []
    rendered_by_source_page: dict[tuple[str, int], dict[str, Any]] = {}
    for page in page_axis:
        ...
        if (
            ...
            raise _error("Family-37 PDF-visible page source-row binding drifted")
        actual_keys.append(key)
        rendered_by_source_page[(document["source_sha256"], key[1])] = page
    if actual_keys != sorted(expected_grouped):
        raise _error("Family-37 PDF-visible page axis is incomplete or unordered")
    ...
    for source in expected_source_axis:
        ...
        if (
            ...
            raise _error("Family-37 PDF-visible source bytes drifted")
        ...
        with fitz.open(path) as pdf:
            for page in pages:
                ...
                if physical_page > len(pdf):
                    raise _error("Family-37 PDF-visible page is outside source PDF")
                ...
                if (
                    ...
                    raise _error("Family-37 PDF-visible page render drifted")
    ...
    if (
        ...
        raise _error("Family-37 manual PDF review axis is absent or drifted")
    ...
    if value.get("counts") != expected_counts:
        raise _error("Family-37 PDF-visible source-row audit counts drifted")
    return {
...
def build_experimental_audit_v1(
    *,
    ...
    source_row_coverage: dict[str, Any],
    source_repair_authentication: dict[str, Any],
    ...
    material = {
        ...
        "family_ownership_receipt": canonical_clone_v1(ownership_receipt),
        "family_id": FAMILY_ID,
...
def validate_experimental_audit_content_v1(value: Any) -> dict[str, Any]:
    if (
        ...
        != NOT_APPLICABLE_DISJOINT_CORPUS
        or value.get("family_ownership_receipt", {}).get("disposition")
        != FAMILY_OWNERSHIP_DISPOSITION
        or value.get("family_ownership_receipt", {}).get(
            "shared_file_mutation_count"
        )
        != 0
        ...
        or value.get("source_repair_authentication_receipt", {}).get(
            "authenticated"
        )
        is not True
        or value.get("source_repair_authentication_receipt", {}).get(
        ...
        raise _error("Family-37 experimental audit content is invalid")
    ...
    if value.get("audit_id") != (
        ...
        raise _error("Family-37 experimental audit identity drifted")
    return canonical_clone_v1(value)


def replay_credit_risk_provision_expense_trials_from_source_v1(
    *,
    ...
    """Re-query, adapt, evaluate, and replay the complete Family-37 axis."""
    ...
    source_repair = generic._json(SOURCE_REPAIR_SPEC_PATH)
    ...
    if not same_typed_json_v1(base_compiled, compiled_specs):
        raise _error("Family-37 source replay declarative specs drifted")
    family_compiled = (
        compile_gemini_json_credit_risk_provision_expense_family_specs_v1(
            topology, evaluation, schema, source_repair
        )
    )
    ...
    if not same_typed_json_v1(indexed, indexed_query_evidence):
        raise _error("Family-37 source replay rebuilt different query evidence")
    trials = build_gemini_json_credit_risk_provision_expense_trials_v1(
        indexed_query_evidence=indexed,
        page_json_by_document=pages,
        compiled_specs=family_compiled,
    )
    return validate_gemini_json_credit_risk_provision_expense_replay_v1(
        base_indexed_query_evidence=base_indexed,
        trials=trials,
        indexed_query_evidence=indexed,
        page_json_by_document=pages,
...
def _run_with_database(
    ...
    *,
    ...
    ownership_receipt = {
        "disposition": FAMILY_OWNERSHIP_DISPOSITION,
        "owned_report_norm_ids": sorted(
            {
                compiled["schema"]["family_root_report_norm_id"],
    ...
    implementation_paths = (
        ROOT
        / "scripts/experiments/run_gemini_json_credit_risk_provision_expense_accounting_family_v1.py",
        ROOT
        / "src/bctc_ai/evaluation/gemini_json_credit_risk_provision_expense_family_v1.py",
        SHARED_EVALUATOR_PATH,
        SHARED_RUNNER_PATH,
    ...
    stored = ingest_gemini_accounting_family_sweep_v1(
        ...
        source_replay_adapter=(
            replay_credit_risk_provision_expense_trials_from_source_v1
        ),
    ...
    if not same_typed_json_v1(stored_sweep, sweep):
        raise _error("Stored Family-37 sweep differs from authenticated evaluation")
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    if args.historical_comparator_policy != DISJOINT_EXPANSION:
        raise _error("Family-37 current-corpus runner requires DISJOINT_EXPANSION")
    ...
    source_repair = generic._json(args.source_repair_spec)
    compiled = (
        compile_gemini_json_credit_risk_provision_expense_family_specs_v1(
            topology, evaluation, schema, source_repair
        )
