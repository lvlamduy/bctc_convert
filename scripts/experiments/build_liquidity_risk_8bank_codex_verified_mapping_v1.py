"""Verify the accounting core of liquidity-risk tables across eight reports."""
...
def _load(name: str, filename: str) -> ModuleType:
    ...
    if spec is None or spec.loader is None:
        raise RuntimeError(f"cannot load liquidity-risk support: {path}")
...
other = _load(
    "other_activity_support_for_liquidity_risk",
    "build_other_activity_8bank_codex_verified_mapping_v1.py",
)
scanner = _load(
    "liquidity_risk_scan_for_verified_mapping",
    "scan_liquidity_risk_full_document_vietocr_v1.py",
)
...
CLAIM_BOUNDARY = (
    ...
    "VIETOCR_BANK_BLIND_LIQUIDITY_RISK_GRAPH_AUTHENTICATED_SOURCE_NUMERIC_"
    "CHALLENGER_GEOMETRIC_COLUMN_ASSIGNMENT_EXACT_ASSET_MINUS_LIABILITY_"
    "EQUALS_NET_GAP_EQUATIONS_LIVE_TM_SCHEMA_CORE_ONLY_ROTATED_"
    "NUMERIC_OR_NONCLOSING_CELLS_RETAINED_NO_EXPORT_AUTHORITY"
)
RESULT_PATH = Path("docs/experiments/E-0103-liquidity-risk-8bank-codex-verified-mapping-v1.json")
...
EXPECTED_SCAN_ID = "lrrfdsv1:scan:04d7232a70833a5b5cd7dbbff64c8c5f3bb514e6c530e00e9dab6b31a5758e0a"

_ROLE_SCHEMA = {
    "OVERDUE": {
        "PARENT": 5898,
        "ASSET_TOTAL": 5899,
        "LIABILITY_TOTAL": 5913,
        "NET_LIQUIDITY_GAP": 5922,
    },
    "OVERDUE_GT3M": {
        "PARENT": 1760,
        "ASSET_TOTAL": 1761,
        "LIABILITY_TOTAL": 1773,
        "NET_LIQUIDITY_GAP": 1782,
    },
    "OVERDUE_LE3M": {
        "PARENT": 1783,
        "ASSET_TOTAL": 1784,
        "LIABILITY_TOTAL": 1796,
        "NET_LIQUIDITY_GAP": 1805,
    },
    "WITHIN_LE1M": {
        "PARENT": 1806,
        "ASSET_TOTAL": 1807,
        "LIABILITY_TOTAL": 1819,
        "NET_LIQUIDITY_GAP": 1828,
    },
    "WITHIN_1_3M": {
        "PARENT": 1829,
        "ASSET_TOTAL": 1830,
        "LIABILITY_TOTAL": 1842,
        "NET_LIQUIDITY_GAP": 1851,
    },
    "WITHIN_3_12M": {
        "PARENT": 1852,
        "ASSET_TOTAL": 1853,
        "LIABILITY_TOTAL": 1865,
        "NET_LIQUIDITY_GAP": 1874,
    },
    "WITHIN_1_5Y": {
        "PARENT": 1875,
        "ASSET_TOTAL": 1876,
        "LIABILITY_TOTAL": 1888,
        "NET_LIQUIDITY_GAP": 1897,
    },
    "WITHIN_GT5Y": {
        "PARENT": 1898,
        "ASSET_TOTAL": 1899,
        "LIABILITY_TOTAL": 1911,
        "NET_LIQUIDITY_GAP": 1920,
    },
    "TOTAL": {
        "PARENT": 1921,
        "ASSET_TOTAL": 1922,
        "LIABILITY_TOTAL": 1934,
        "NET_LIQUIDITY_GAP": 1943,
    },
}
_EXPECTED_ROLE_NAMES = {
    "ASSET_TOTAL": "Tổng Tài sản",
    "LIABILITY_TOTAL": "Tổng Nợ phải trả",
    "NET_LIQUIDITY_GAP": "Chênh lệch thanh khoản ròng",
}
...
_AUTHORITY = {
    ...
    "maturity_axis_or_row_order_required": False,
...
class LiquidityRisk8BankCodexVerifiedMappingV1Error(ValueError):
    """The graph, values, equations, schema or result drifted."""


def _error(message: str) -> LiquidityRisk8BankCodexVerifiedMappingV1Error:
    return LiquidityRisk8BankCodexVerifiedMappingV1Error(message)
...
def _schema_family(item: Any) -> dict[str, Any]:
    if (
        ...
        or item.display_order != 1483
    ):
        raise _error("liquidity-risk family does not bind exact live TM schema root")
    return {
...
def _schema_binding(item: Any, axis: str, role: str) -> dict[str, Any]:
    ...
    if (
        ...
        raise _error(f"mapping does not bind exact live liquidity-risk TM row {report_norm_id}")
    return {
        "canonical_name": item.canonical_name,
        "display_order": item.display_order,
        "hierarchy_level": item.hierarchy_level,
...
def _period(page: Mapping[str, Any]) -> tuple[str, str, str]:
    ...
    for line in page["lines"]:
        ...
        elif re.search(r"31\s+(?:thang\s+)?12(?:\s+nam)?\s+202[5s]", value):
    ...
    if re.search(r"31\s+(?:thang\s+)?12(?:\s+nam)?\s+202[5s]", text):
        return "COMPARATIVE", "2025-12-31", "VERIFIED_COMPARATIVE_2025_12_31"
    raise _error("liquidity-risk table period could not be derived from the bound page")
...
def _label_spans(page: Mapping[str, Any], matcher: ModuleType) -> dict[str, dict[str, Any]]:
    ...
    for index, line in enumerate(labels):
        ...
        explored = [line]
        for following in labels[index + 1 : index + 3] if role is None else []:
    ...
    if asset_total_y is None:
        raise _error("liquidity-risk table has no geometric asset-total boundary")
...
def _label_evidence(
    ...
    for line in span["parts"]:
        ...
        if semantic_line["source_line_index"] != source_index:
            raise _error("liquidity-risk label semantic line axis drifted")
        components.append(
            {
...
def _row_cells(
    ...
    for line in page["lines"]:
        ...
        except LiquidityRisk8BankCodexVerifiedMappingV1Error:
            continue
        candidates.append(
            {
    ...
    if len(centers) < 2:
        raise _error("liquidity-risk table has too few geometric axes")
    assigned: dict[str, dict[str, Any]] = {}
    for cell in selected:
        index = min(range(len(centers)), key=lambda item: abs(centers[item] - cell["center_x"]))
        neighbour_spacings = []
        if index:
            neighbour_spacings.append(centers[index] - centers[index - 1])
        if index + 1 < len(centers):
            neighbour_spacings.append(centers[index + 1] - centers[index])
        local_spacing = min(neighbour_spacings)
        if abs(centers[index] - cell["center_x"]) > local_spacing * 0.52:
            raise _error(
                "liquidity-risk value is outside every maturity column: "
                f"role={span['role']}, source_line={cell['line']['source_line_index']}, "
                f"x={cell['center_x']}, nearest={centers[index]}"
            )
        axis = axis_centers[index][0]
        if axis in assigned:
            raise _error("liquidity-risk row repeats one maturity column")
        assigned[axis] = cell
    return assigned
...
def _value_evidence(
    ...
    *,
    ...
    if evidence["normalized_value"] != cell["normalized_value"]:
        raise _error("liquidity-risk source numeric normalization drifted")
...
def _parsed_table(
    ...
    axis_event_by_role = {
        event["role"].removeprefix("MATURITY_AXIS_"): event
        for event in axis_events
        if event["role"].startswith("MATURITY_AXIS_")
    }
    if set(axis_event_by_role) != set(axes):
        raise _error("liquidity-risk header geometry or axis roles drifted")
    axis_centers = [
        (
            axis,
    ...
    if not {"ASSET_TOTAL", "LIABILITY_TOTAL", "NET_LIQUIDITY_GAP"} <= set(spans):
        raise _error("liquidity-risk table lost one required core row")
...
def _equations(table: Mapping[str, Any]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    exact = []
    residuals = []
    for axis in table["maturity_axes"]:
        roles = ("ASSET_TOTAL", "LIABILITY_TOTAL", "NET_LIQUIDITY_GAP")
        ...
        record = {
            ...
            "equation_kind": "ASSET_TOTAL_MINUS_LIABILITY_TOTAL_EQUALS_NET_LIQUIDITY_GAP",
...
def _eligible_cells(
    ...
    for equation in equations:
        roles = ("ASSET_TOTAL", "LIABILITY_TOTAL", "NET_LIQUIDITY_GAP")
        eligible.update((equation["maturity_axis"], role) for role in roles)
    return eligible
    ...
    *,
        ...
        return (
            {
                **base,
                "absence_evidence": {
                    ...
                    "reason": "NO_COMPLETE_LIQUIDITY_RISK_TABLE_IN_BOUND_REPORT",
                },
    ...
    for table in tables:
        ...
        residual_axes_by_page[table["page_sequence"]].update(
            residual["maturity_axis"] for residual in table_residuals
        )
    ...
    for key in sorted(source_only):
        ...
        verified_source_only_rows.append({**group, "gap_id": f"LRISK-{next_gap_number:03d}"})
        next_gap_number += 1
...
def _validate_result(value: Any) -> dict[str, Any]:
    if type(value) is not dict or set(value) != _RESULT_FIELDS:
        raise _error("liquidity-risk result fields drifted")
    if (
        value["format_version"] != FORMAT_VERSION
        or value["claim_boundary"] != CLAIM_BOUNDARY
        or value["state"] != "LIQUIDITY_RISK_8BANK_CODEX_VERIFICATION_COMPLETE"
        or not same_typed_json_v1(value["authority"], _AUTHORITY)
        or type(value["trials"]) is not list
        or len(value["trials"]) != len(EXPECTED_DOCUMENT_ORDER)
        or not same_typed_json_v1(value["metrics"], _metrics(value["trials"]))
    ):
        raise _error("liquidity-risk result identity or metrics drifted")
    gap_ids = []
    for ordinal, (trial, code) in enumerate(
        ...
        if (
            ...
            raise _error("liquidity-risk trial shape or status drifted")
        gap_ids.extend(row["gap_id"] for row in trial.get("verified_source_only_rows", []))
    if gap_ids != [f"LRISK-{index:03d}" for index in range(1, len(gap_ids) + 1)]:
        raise _error("liquidity-risk source gap IDs drifted")
    material = canonical_clone_v1(value)
    identity = material.pop("result_id")
    if identity != "e0103:result:" + canonical_json_sha256_v1(material):
        raise _error("liquidity-risk result identity drifted")
    return canonical_clone_v1(value)


def build_liquidity_risk_8bank_codex_verified_mapping_v1(
    semantic_index: Any,
    crop_manifest: Any,
    structure_scan: Any,
    rescue: Any,
    ...
    *,
    ...
    scanner.validate_liquidity_risk_full_document_scan_replay_v1(
        structure_scan, semantic_index, rescue
    )
    if structure_scan["scan_id"] != EXPECTED_SCAN_ID:
        raise _error("liquidity-risk fixed scan ID drifted")
    axis = project_full_document_vietocr_accounting_axis_v1(semantic_index)
    if axis["semantic_axis_sha256"] != EXPECTED_AXIS_SHA256:
        raise _error("liquidity-risk semantic axis drifted")
    ...
    for axis_document, semantic_document, crop_document, scan_trial in zip(
        ...
        if applied != scan_trial["rotated_rescue_line_count"]:
            raise _error("liquidity-risk rotated rescue trial binding drifted")
    ...
    material = {
        ...
        "state": "LIQUIDITY_RISK_8BANK_CODEX_VERIFICATION_COMPLETE",
        "trials": trials,
    }
    return _validate_result(
        {**material, "result_id": "e0103:result:" + canonical_json_sha256_v1(material)}
    )


def validate_liquidity_risk_8bank_codex_verified_mapping_replay_v1(
    value: Any,
    semantic_index: Any,
    crop_manifest: Any,
    structure_scan: Any,
    ...
    *,
    ...
    rebuilt = build_liquidity_risk_8bank_codex_verified_mapping_v1(
        semantic_index,
        crop_manifest,
        structure_scan,
        rescue,
    ...
    if not same_typed_json_v1(supplied, rebuilt):
        raise _error("liquidity-risk verified mapping does not replay exactly")
    return supplied
...
def _live_inputs() -> dict[str, Any]:
    ...
    structure_scan = scanner.build_liquidity_risk_full_document_scan_v1(semantic_index, rescue)
...
def build_live_liquidity_risk_8bank_codex_verified_mapping_v1() -> dict[str, Any]:
    return build_liquidity_risk_8bank_codex_verified_mapping_v1(**_live_inputs())


def validate_live_liquidity_risk_8bank_codex_verified_mapping_v1(
    value: Any,
) -> dict[str, Any]:
    return validate_liquidity_risk_8bank_codex_verified_mapping_replay_v1(value, **_live_inputs())


def _write(path: Path, value: Any) -> None:
    destination = PROJECT_ROOT / path
    if destination.exists():
        raise _error(f"refusing to overwrite existing liquidity-risk result: {path}")
...
def main() -> None:
    ...
    if args.write_result:
        _write(RESULT_PATH, build_live_liquidity_risk_8bank_codex_verified_mapping_v1())
        return
    result, _ = _stable_json(RESULT_PATH)
    validate_live_liquidity_risk_8bank_codex_verified_mapping_v1(result)
