"""Scan all eight fresh-VietOCR PDFs for currency-risk tables."""
...
FORMAT_VERSION = "CURRENCY_RISK_8DOCUMENT_FULL_VIETOCR_SCAN_V1"
MATCHER_FORMAT = "CURRENCY_RISK_VARIANT_GRAPH_DOCUMENT_V1"
CLAIM_BOUNDARY = (
    "FRESH_VIETOCR_TRANSFORMER_COMPLETE_PDF_BANK_BLIND_CURRENCY_RISK_"
    "OPTIONAL_AXIS_ASSET_LIABILITY_STATE_SCAN_ONLY_NO_NUMERIC_SCHEMA_"
    "MAPPING_OR_EXPORT_AUTHORITY"
)
_AUTHORITY = {
    "bounded_detailed_table_absence_only": True,
    "complete_pdf_scanned_for_every_document": True,
    "fair_value_interest_and_liquidity_controls_retained": True,
...
class CurrencyRiskFullDocumentScanV1Error(ValueError):
    """The semantic axis or currency-risk scan drifted."""


def _error(message: str) -> CurrencyRiskFullDocumentScanV1Error:
    return CurrencyRiskFullDocumentScanV1Error(message)
...
def _load(name: str, filename: str) -> ModuleType:
    ...
    if spec is None or spec.loader is None:
        raise _error(f"cannot load currency-risk support: {path}")
...
def _support() -> ModuleType:
    return _load(
        "fx_gold_scan_support_for_currency_risk",
...
def _matcher() -> ModuleType:
    return _load(
        "currency_risk_matcher_for_scan",
...
def _validate(value: Any) -> dict[str, Any]:
    if type(value) is not dict or set(value) != _FIELDS:
        raise _error("currency-risk scan fields drifted")
    if (
        value["format_version"] != FORMAT_VERSION
        or value["claim_boundary"] != CLAIM_BOUNDARY
        or value["state"] != "FULL_DOCUMENT_CURRENCY_RISK_SCAN_COMPLETE"
        or not same_typed_json_v1(value["authority"], _AUTHORITY)
        or type(value["trials"]) is not list
        or len(value["trials"]) != len(EXPECTED_DOCUMENT_ORDER)
        or not same_typed_json_v1(value["metrics"], _metrics(value["trials"]))
    ):
        raise _error("currency-risk scan identity drifted")
    for ordinal, (trial, code) in enumerate(
        ...
        if (
            ...
            raise _error("currency-risk trial identity drifted")
    material = canonical_clone_v1(value)
    identity = material.pop("scan_id")
    if identity != "crfdsv1:scan:" + canonical_json_sha256_v1(material):
        raise _error("currency-risk scan ID drifted")
    return canonical_clone_v1(value)


def build_currency_risk_full_document_scan_v1(semantic_index: Any) -> dict[str, Any]:
    ...
    return _validate({**material, "scan_id": "crfdsv1:scan:" + canonical_json_sha256_v1(material)})


def validate_currency_risk_full_document_scan_replay_v1(
    value: Any, semantic_index: Any
) -> dict[str, Any]:
    supplied = _validate(value)
    rebuilt = build_currency_risk_full_document_scan_v1(semantic_index)
    if not same_typed_json_v1(supplied, rebuilt):
        raise _error("currency-risk scan does not replay exactly")
    return supplied


def build_live_currency_risk_full_document_scan_v1(
    input_path: Path = DEFAULT_INPUT,
) -> dict[str, Any]:
    support = _support()._support()
    semantic_index = support._strict_json(support._stable_bytes(input_path), input_path.as_posix())
    return build_currency_risk_full_document_scan_v1(semantic_index)
...
def main() -> None:
    ...
    sys.stdout.buffer.write(
        canonical_json_bytes_v1(build_live_currency_risk_full_document_scan_v1(args.input))
    )
