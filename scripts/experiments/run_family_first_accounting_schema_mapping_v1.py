#!/usr/bin/env python3
"""Build or replay one generic all-filing accounting schema mapping."""
...
from bctc_ai.evaluation.family_first_accounting_schema_mapping_v1 import (  # noqa: E402
...
OUTPUT_ROOT = Path("output/calibration/family-first-accounting-schema-mappings-v1")


class FamilyFirstAccountingSchemaMappingCliV1Error(RuntimeError):
    """The tracked specs, authenticated indices, schema, or artifact drifted."""


def _error(message: str) -> FamilyFirstAccountingSchemaMappingCliV1Error:
    return FamilyFirstAccountingSchemaMappingCliV1Error(message)
...
def _artifact_path(
    ...
    if (
        ...
        raise _error("family/evaluation/schema specification identities differ or are unsafe")
    return OUTPUT_ROOT / (family_id.lower().replace("_", "-") + ".json")


def run_family_first_accounting_schema_mapping_v1(
    project_root: Path,
    *,
    model_cache: Path,
    ...
    """Build or exact-replay one fixed family mapping artifact."""

    if command not in {"build", "verify"}:
        raise _error("accounting schema mapping command must be build or verify")
    ...
    output_relative = _artifact_path(family_spec, evaluation_spec, schema_binding_spec)
    ...
    if command == "build":
        result = build_authenticated_family_first_accounting_schema_mapping_v1(
            root,
            semantic_index,
            numeric_index,
            family_spec,
...
def main() -> int:
    ...
    result = run_family_first_accounting_schema_mapping_v1(
        PROJECT_ROOT,
