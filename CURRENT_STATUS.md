...
| TM | 1,385 | 82/1,385 | 55 PDF rows on pages 30–31; 96 cells | 46/1,385 | 2 ambiguous IDs; 8 source-only/validation rows; 1,303 unassessed | 0/1,385 |

CDKT reconciles exactly as `77 = 61 MAPPED + 15 NOT_OBSERVED_IN_THIS_PDF + 1 UNRESOLVED`; its three source-only PDF rows are outside the denominator. KQKD reconciles as `24 = 21 MAPPED + 3 NOT_OBSERVED_IN_THIS_PDF`, plus one source-only total row. Its 88/88 observed values agree with the independent PDF text layer and 32/32 accounting checks have zero residual. LCTT reconciles as `107 = 57 SCHEMA_ITEM_NOT_APPLICABLE + 40 MAPPED + 1 LABEL_CONFLICT_CANDIDATE_NOT_AUTOMATIC + 5 AMBIGUOUS_MAPPING + 4 NOT_OBSERVED_IN_THIS_PDF`; its two net/composite rows remain source-only. TM pages 30–31 reconcile as `1,385 = 46 MAPPED + 2 AMBIGUOUS_MAPPING + 11 NOT_OBSERVED_IN_THIS_PDF + 23 SCHEMA_ITEM_NOT_APPLICABLE + 1,303 UNASSESSED`; eight source-only/validation rows sit outside the denominator. “Fully verified” remains zero until the complete mapping/value/period/status/Excel tuple has independent authority.
...
CDKT uses the sealed semantic-normalization/role-repair path. KQKD uses a four-axis hierarchical header parser and history-free SchemaGraph mapping. LCTT uses a two-page direct-method parser plus ordered PP-OCR/DeepSeek corroboration; 40 one-to-one rows are auto-mapped and one visible/schema label conflict is withheld. TM pages 30–31 use repeated-note fixed-grid geometry plus PP-OCR/DeepSeek labels; page 31 adds hierarchy-aware duplicate suppression and 14 zero-residual accounting checks. Qwen is not in the active path.
...
- **Role B:** MBB CDKT reaches a hash-sealed, S3-restored template workbook. MBB KQKD reaches deterministic development Excel with 21/24 mappings; MBB LCTT reaches deterministic development Excel with 40/107 mappings and the conflicting 4140 row withheld. TM pages 30–31 now reach 55 rows/96 cells and 46 scoped mappings; repeated totals are validation-only rather than duplicated in schema targets.
...
- **Regression status:** cross-statement KQKD/LCTT/TM coverage regression 50/50 passed; Ruff check/format passed. Latest full parser regression: 983 passed, 2 expected skips.
- **Biggest remaining blocker:** 1,303 TM schema items remain unassessed, alongside 14 concrete TM questions exposed in `QUESTION_FOR_USER.md`; mechanism sealing is not the limiting work.
- **Exact next end-to-end step:** promote the simpler two-axis TM page 35 audit into safe mappings, then pages 32–33 amount/percentage tables and page 34 nine-axis provision movements.
...
Completed through: sealed CDKT Excel; KQKD and LCTT development Excel; TM pages 30–31 item mapping
Currently working on: TM pages 32–35, then the remaining quantitative-note pages through 61
