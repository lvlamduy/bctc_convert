...
- Family 45 `BANK_PLEDGED_OR_DISCOUNTED_ASSETS`: `READY=50`,
  `NOT_OBSERVED=90`, `UNRESOLVED=0` và 114 mappings. Generic multi-table
  hierarchical resolver inventory đúng bank-owned owner/reset fence, khóa hai
  period lane, unit, source total, direct child frontier và hierarchy
  `Trong đó` mà không route theo bank/file/page. Customer collateral,
  borrowing-facility text và policy mention chỉ là controls. Prompt Gemini vẫn
  tối giản, cố định; graph, equation, hierarchy và mapping đều do thuật toán
  local xây và kiểm chứng.

  Hai primitive opt-in mới giữ tính tổng quát: complete owner MONEY-table có
  thể tạo exact source population; validation-only carrier có thể project vào
  residual leaf chỉ sau khi source equation tiêu thụ toàn bộ frontier. Wrapper
  `Trong đó` được bind bằng source hierarchy. Trường hợp nguồn thật sự in cả
  parent lẫn children vào cùng total không bị sửa hay ép thành một phương trình
  giả: receipt riêng ghi nhận exact overlapping source hierarchy, chỉ cho phép
  carrier validation-only, và vẫn giữ printed root cùng các child không trùng.

  Exhaustive query quét đủ 8.947 selected page versions và sinh 50 accepted
  clusters, 90 typed absence dispositions, 52 equation receipts. E-0097/E-0152
  khớp đủ 16/16 historical dispositions và 18/18 mappings. Public SQLite query
  và candidate replay dựng lại toàn bộ candidate từ canonical page JSON; các
  coherent receipt/spec/candidate drift đều fail closed. Source contradiction
  tại VPB ordinal 137 được đối chiếu trực tiếp với ảnh 300 dpi và giữ nguyên,
  không gọi Gemini repair.

  OFFICIAL sweep là
  `/tmp/gemini-json-first-corpus-production-v2/artifacts/current-family-results/family45-bank-pledged-or-discounted-assets/sweep.json`,
  SHA-256
  `01e0ea23028f32e54e42a913f716e9a22998995e4b04c5143774c774198fdc64`;
  audit SHA-256
  `dd6255035e8b69dba24ea011a9e297921bbe160e2c6e833009cbce0c8fd870ec`.
  Implementation commit `ec53443`; 293 focused/adjacent/regression tests,
  Ruff, format, compile, JSON, diff, EXP/OFFICIAL replay và SQLite integrity
  đều xanh. Cumulative results DB có 45 current selections và 45 selection
...
- Family 48 `CURRENCY_RISK`: `READY=140`, `NOT_OBSERVED=0`,
  `UNRESOLVED=0`, 3.632 mappings, 3.512 mapped values và 1.754 equation
  receipts. Generic currency-matrix resolver tự inventory 176 source fragments,
  khóa asset/liability/internal/external/combined-state rows, currency columns,
  nested totals, continuation, period và unit. Prompt Gemini không đổi và không
  chứa schema, graph, equation, bank/file/page hoặc đáp án family.

  Mười lăm source equations không đóng được giữ nguyên trong typed
  `nonclosing_currency_frontiers`; 143 unsupported currency columns được giữ
  source-only, không bị gộp vào `OTHER`, dùng để chọn candidate hoặc sửa số
  nguồn. Blank chỉ được chiếu zero khi đúng một unknown có nghiệm zero duy nhất;
  nonzero missing value không được backsolve. Partial/duplicate population,
  owner/reset drift, period/unit conflict, source-only collapse, mapping thiếu
  hoặc receipt coherent-rehash đều fail closed.

  Public SQLite query/candidate replay dựng lại toàn bộ candidate từ đúng 8.947
  selected page versions. E-0101/E-0155 khớp 258/258 historical mappings,
  122/122 equations và 18/18 source-only columns; comparator không route theo
  tài liệu. OFFICIAL sweep là
  `/tmp/gemini-json-first-corpus-production-v2/artifacts/current-family-results/family48-currency-risk/sweep.json`,
  SHA-256
  `e5bf48bf0a53972fa1fbcdbb03f32f17f518ef67761f9ff1e724b17fd4f370eb`;
  audit SHA-256
  `4c30d28476148e4f91623a8c5c60cdffc9fa15cc28cf53b196e98588e560de19`,
  audit ID
  `gjfcreav1:audit:b365c74848356dc8c441e5514fe8b86967e0d90892eee0a9856e588f75abd76f`.
  Implementation commit `48319c7`; 100 focused/adjacent/indexed/runner tests
  được chạy lại sau OFFICIAL, source replay 8.947/8.947 exact, EXP/OFFICIAL
  artifacts byte-identical và SQLite integrity đều xanh. Cumulative results DB
  có 48 current selections và 48 selection events, `quick_check`/foreign keys
  sạch, không SQLite sidecar, SHA-256
  `9d9051f1090cdb5b8f54e728bc702a8b4c440ad5e5e89b5959e8a385609e1475`.

## Family 49 closure

- Family 49 `INTEREST_RATE_RISK`: `READY=139`, `NOT_OBSERVED=1`,
  `UNRESOLVED=0`, 6.879 mappings, 6.788 mapped values và 3.118 equation
  receipts. Generic interest-rate matrix resolver tự inventory 173 source
  fragments trong 139 clusters, khóa repricing/maturity buckets,
  asset/liability/internal/external/combined-state rows, period, unit,
  continuation và complete source population. Ordinal 134 không có semantic
  anchor được giữ typed `NOT_OBSERVED`; không tạo mapping giả.

  Bốn vùng khó được xử lý bằng cùng repair contract tối giản
  `{observations:[{cell_id,source_text}]}`. Ba trang có lỗi đọc nguồn được thay
  bằng page versions có content-addressed lineage; một trang được xác nhận là
  source presentation đúng và không tạo replacement. Gemini không nhận graph,
  equation, schema, ReportNormId, bank/file/page rule hoặc đáp án. Thuật toán
  local tự project observation lên JSON bất biến, kiểm exact row graph, source
  totals và phương trình rồi mới cho phép mapping. Final replay tái sử dụng tám
  sealed observations, gọi provider mới bằng zero.

  Effective frontier `gjfepfv1:frontier:b5d5597e2b67a04fbf8bc3cc4809e880489fd1b19ac6b0d7346b5ff083ca94d2`
  thay đúng ba trong 8.947 selected page versions. Public SQLite query/candidate
  replay dựng lại toàn bộ candidate từ repaired canonical pages; coherent
  source-receipt drift sau khi rehash bị loại. E-0102/E-0156 khớp 429/429
  historical mappings và 141/141 historical equations. Hai legacy absences
  được source JSON hiện tại supersede minh bạch; 214 nonclosing source
  frontiers và 28 source-only columns vẫn được receipted, không bị backsolve hay
  gộp tùy ý.

  OFFICIAL sweep là
  `/tmp/gemini-json-first-corpus-production-v2/artifacts/current-family-results/family49-interest-rate-risk/sweep.json`,
  SHA-256
  `4e7d53f405fbbe0e3c3db5d977de667037b60b0d3956c57284d52defef0e6449`;
  audit SHA-256
  `087203145c11d9ee2c8ac885b0fa6e8705fe85a62c565cf0f8cb5e565a938447`,
  audit ID
  `gjfirreav1:audit:a3a4da31b39926f94b5d61b7739b69f5aa4ebac1bb2f60d387eec4887711bc13`.
  Implementation commits `a076259`, `978b9a1`, `78b3067` và `f9c0dc3`;
  63 focused/adjacent/indexed/store/runner tests, Ruff, format, diff, exact
  SQLite replay, coherent-tamper rejection và byte-identical EXP/OFFICIAL
  artifacts đều xanh. Cumulative results DB có 49 current selections và 49
  selection events, `quick_check`/foreign keys sạch, không SQLite sidecar,
  SHA-256
