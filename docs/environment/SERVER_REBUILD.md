...
9. Build the isolated GPU environment only from the pinned model-runtime manifest. Run a real CUDA kernel smoke test before downloading model weights.
...
11. Set `BCTC_GPU_UV_CACHE_DIR` to a filesystem with at least 8 GiB free and run `scripts/bootstrap/create_gpu_runtime.sh`. Its freeze diff, dependency check, import check, `sm_120` check, and real CUDA kernel must all pass.
...
GPU-specific commands, disk budgets, precision settings, cache variables, failure history, and rollback are in `GPU_RUNTIME_RUNBOOK.md`. Historical-reference source selection, rebuild, constraints, and query rules are in `HISTORICAL_REFERENCE_RUNBOOK.md`. Model weights, virtual environments, Mongo data, and the local DuckDB file are reconstructed rather than committed to Git.
