"""Crash-bounded GPU reference-blind PP-OCRv6 numeric cache.
...
reader axis and recognizer were sound.  V3 partitions that same immutable,
...
V3 also pins the CUDA distribution and exact RTX 4090 compute identity.  The
recognizer still receives no bank,
...
__all__ = [
    ...
    "CACHE_ROOT",
    "SHARD_SIZE",
    "FamilyFirstPPocrV6NumericShardedRunnerV3Error",
...
CACHE_ROOT = Path("output/calibration/family-first-ppocrv6-numeric-cache-v3")
SHARDS_ROOT = CACHE_ROOT / "shards"
AGGREGATE_ROOT = CACHE_ROOT / "aggregate"
SHARD_SIZE = 2048
EXPERIMENT_ID = "FAMILY_FIRST_ALL_FILING_PPOCRV6_NUMERIC_GPU_SHARDED_CACHE_V3"
SHARD_FORMAT_VERSION = "FAMILY_FIRST_PPOCRV6_NUMERIC_COMPLETED_SHARD_V3"
AGGREGATE_FORMAT_VERSION = "FAMILY_FIRST_PPOCRV6_NUMERIC_AGGREGATE_V3"
    ...
    sorted(
        {
            *(
            ...
            Path("src/bctc_ai/ocr/family_first_ppocrv6_numeric_sharded_runner_v3.py"),
            Path("src/bctc_ai/evaluation/family_first_ppocrv6_numeric_index_v3.py"),
            Path("scripts/experiments/run_family_first_ppocrv6_numeric_v3.py"),
        },
...
class FamilyFirstPPocrV6NumericShardedRunnerV3Error(RuntimeError):
    """One shard, the complete shard axis, or its aggregate drifted."""


def _error(message: str) -> FamilyFirstPPocrV6NumericShardedRunnerV3Error:
    return FamilyFirstPPocrV6NumericShardedRunnerV3Error(message)
...
def _git_binding(root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "dirty": False,
        "implementation_refs": [
            runtime_v3._tracked_ref(root, path, f"numeric V3 trust file {path}")
...
def _git_ledger(root: Path, binding: Any) -> str:
    ...
    if (
        ...
        raise _error("numeric V3 Git binding drifted")
    ...
    try:
        ...
        raise _error("numeric V3 run commit is not an ancestor of HEAD") from exc
    for raw in binding["implementation_refs"]:
        try:
            reference = archive_v1._ref(raw, "numeric V3 trust file")
        except archive_v1.FamilyFirstSemanticLabelArchiveV1Error as exc:
            raise _error("numeric V3 trust-file reference drifted") from exc
        committed = archive_v1._git(root, "show", f"{run_commit}:{reference['path']}")
        current = archive_v1._git(root, "show", f"{head}:{reference['path']}")
        disk = _root_bytes(root, Path(reference["path"]), "numeric V3 trust file")
        if (
            ...
            raise _error("numeric V3 trust file changed on the descendant chain")
    run_tree = (
    ...
    if run_tree != binding["source_tree_oid"]:
        raise _error("numeric V3 source-tree identity drifted at its run commit")
    if archive_v1._clean_head(root) != head:
        raise _error("Git HEAD/worktree changed during numeric V3 ledger replay")
    return head
...
def _validate_runtime(value: Any, model: dict[str, Any]) -> dict[str, Any]:
    expected_packages = {"paddleocr": "3.7.0", "paddlepaddle-gpu": "3.3.0"}
    ...
    if (
        ...
        or set(value) != {"accelerator", "device", "model", "packages", "precision"}
        or not same_typed_json_v1(value["accelerator"], expected_accelerator)
        or value["device"] != "gpu:0"
...
def _context(
    ...
    *,
    ...
    model, _directory = kernel_v1._recognizer_projection(
        root,
        model_cache,
...
def _run_shard(
    ...
    *,
    ...
    try:
        ...
        manifest = {
            **material,
            "shard_id": "ffpnsv3:shard:" + canonical_json_sha256_v1(material),
        }
...
def _validate_shard(
    ...
    if (
        ...
        or shard_id != "ffpnsv3:shard:" + canonical_json_sha256_v1(material)
...
def run_authenticated_family_first_ppocrv6_numeric_shard_v3(
    project_root: Path,
    archive_capability: archive_v1.AuthenticatedFamilyFirstSemanticLabelArchiveV1,
    *,
    model_cache: Path,
    ...
    if not isinstance(model_cache, Path):
        raise _error("numeric V3 model cache must be one pathlib Path")
...
def run_authenticated_family_first_ppocrv6_numeric_missing_shards_v3(
    project_root: Path,
    archive_capability: archive_v1.AuthenticatedFamilyFirstSemanticLabelArchiveV1,
    *,
    model_cache: Path,
    ...
    return {
        ...
        "format_version": "FAMILY_FIRST_PPOCRV6_NUMERIC_SHARD_PROGRESS_V3",
...
def project_authenticated_family_first_ppocrv6_numeric_shards_v3(
    project_root: Path,
    archive_capability: archive_v1.AuthenticatedFamilyFirstSemanticLabelArchiveV1,
    *,
    model_cache: Path,
...
def aggregate_authenticated_family_first_ppocrv6_numeric_v3(
    project_root: Path,
    archive_capability: archive_v1.AuthenticatedFamilyFirstSemanticLabelArchiveV1,
    *,
    model_cache: Path,
    ...
    if os.path.lexists(root / AGGREGATE_ROOT):
        raise _error("fixed numeric V3 aggregate already exists")
    context = _context(root, archive_capability, model_cache, open_reader=False)
    total = _shard_count(context["projection"]["sample_count"])
    cache_fd, shards_fd = _cache_fds(root)
    ...
    try:
        ...
        aggregate = {
            **material,
            "aggregate_id": "ffpnav3:aggregate:" + canonical_json_sha256_v1(material),
        }
        file_ops_v1._write_exclusive(
...
def validate_authenticated_family_first_ppocrv6_numeric_aggregate_v3(
    project_root: Path,
    archive_capability: archive_v1.AuthenticatedFamilyFirstSemanticLabelArchiveV1,
    *,
    model_cache: Path,
    ...
    expected = {
        **expected_material,
        "aggregate_id": "ffpnav3:aggregate:" + canonical_json_sha256_v1(expected_material),
    }
