"""Hash-bound fixed-grid reconstruction for MBB consolidated TM page 60.

The page contains one liquidity-risk table with seven maturity axes. Printed
dashes are retained only when constrained pixel evidence exists; the low-score
false glyph at a dash location is rejected and never promoted to a value.
"""
...
_AXIS_RIGHT_EDGES = (1220.0, 1570.0, 1912.0, 2251.0, 2589.0, 2935.0, 3273.0)
...
_EXPECTED_NUMERIC_GROUP_SIZES = (2, 2, 6, 2, 7, 7, 2, 2, 7, 7, 3, 5, 6, 5, 5, 6, 6, 6, 7)
_EXPECTED_ARTIFACT_INDICES: tuple[int, ...] = ()
_EXPECTED_REJECTED_LOW_SCORE_INDICES = (17,)
_REQUIRED_FORBIDDEN = {
...
class TMPage60Policy:
    ...
    axes: tuple[TMPage60AxisSpec, ...]
    rows: tuple[TMPage60RowSpec, ...]
    ...
    page_60_closes_liquidity_table: bool
    page_61_starts_topic: str
    forbidden_semantic_inputs: tuple[str, ...]


@dataclass(frozen=True)
class TMPage60AxisBinding:
    ordinal: int
    axis_key: str
    canonical_label: str
...
class TMPage60Table:
    ...
    axes: tuple[TMPage60AxisBinding, ...]
    rows: tuple[TMPage60LogicalRow, ...]
    bbox: BoundingBox


@dataclass(frozen=True)
class ParsedTMPage60:
    input_path: str
    source_sha256: str
    upstream_ocr_sha256: str
    source_render_sha256: str
...
def _anchors(value: Any, field: str) -> tuple[str, ...]:
    if not isinstance(value, list) or not value:
        raise TMNoteWordBoxError(f"TM page-60 {field} anchors are invalid")
    result = tuple(retrieval_key(str(item)) for item in value)
    if any(not item for item in result):
        raise TMNoteWordBoxError(f"TM page-60 {field} contains an empty anchor")
    return result


def _anchor_groups(value: Any, field: str) -> tuple[tuple[str, ...], ...]:
    if not isinstance(value, list) or not value:
        raise TMNoteWordBoxError(f"TM page-60 {field} groups are invalid")
...
def _date_value(payload: dict[str, Any], field: str) -> date:
    ...
    if isinstance(value, str):
        try:
            ...
            raise TMNoteWordBoxError(f"invalid TM page-60 {field}") from exc
    raise TMNoteWordBoxError(f"invalid TM page-60 {field}")


def _positive(payload: dict[str, Any], field: str, *, upper: float | None = None) -> float:
    value = payload.get(field)
    if isinstance(value, bool) or not isinstance(value, (int, float)) or value <= 0:
        raise TMNoteWordBoxError(f"invalid positive TM page-60 setting: {field}")
    result = float(value)
    if upper is not None and result >= upper:
        raise TMNoteWordBoxError(f"TM page-60 {field} must be below {upper}")
    return result


def load_tm_page60_policy(path: Path) -> TMPage60Policy:
    """Load the immutable source-scoped page-60 reconstruction contract."""

    try:
        payload = yaml.safe_load(path.read_text(encoding="utf-8"))
    except (OSError, yaml.YAMLError) as exc:
        raise TMNoteWordBoxError(f"cannot load TM page-60 policy: {path}") from exc
    if not isinstance(payload, dict) or (
        payload.get("version") != 1
        or payload.get("policy") != "TM_NOTE_PAGE60_LIQUIDITY_RISK_FIXED_GRID_DASH_V1"
        ...
        raise TMNoteWordBoxError("TM page-60 policy identity drifted")
    hash_fields = (
        "source_pdf_sha256",
    ...
    if any(not isinstance(value, str) or not _SHA256.fullmatch(value) for value in hashes):
        raise TMNoteWordBoxError("TM page-60 source hashes are invalid")
    thresholds = tuple(
        payload.get(field)
    ...
    if any(
        ...
        raise TMNoteWordBoxError("TM page-60 similarity thresholds are invalid")
    ...
    if not all(isinstance(item, dict) for item in (unit, reporting, table, geometry, continuation)):
        raise TMNoteWordBoxError("TM page-60 unit/period/table policy is incomplete")
    ...
    if (
        ...
        raise TMNoteWordBoxError("TM page-60 unit binding is invalid")
    if canonical_unit != "VND" or multiplier != 1_000_000:
        raise TMNoteWordBoxError("TM page-60 native VND-million unit drifted")
    if reporting.get("period_role") != "CURRENT" or reporting.get("period_type") != "SNAPSHOT":
        raise TMNoteWordBoxError("TM page-60 period role/type drifted")
    ...
    if (
        ...
        raise TMNoteWordBoxError("TM page-60 snapshot date drifted")

    raw_axes = payload.get("axes")
    if not isinstance(raw_axes, list) or len(raw_axes) != 7:
        raise TMNoteWordBoxError("TM page-60 must define seven axes")
    axes = []
    for record in raw_axes:
        if not isinstance(record, dict):
            raise TMNoteWordBoxError("TM page-60 axis is invalid")
        right_edge = record.get("right_edge")
        if isinstance(right_edge, bool) or not isinstance(right_edge, (int, float)):
            raise TMNoteWordBoxError("TM page-60 axis edge is invalid")
        axes.append(
            TMPage60AxisSpec(
    ...
    if (
        ...
        raise TMNoteWordBoxError("TM page-60 axis order/geometry drifted")

    raw_rows = payload.get("rows")
    if not isinstance(raw_rows, list) or len(raw_rows) != 22:
        raise TMNoteWordBoxError("TM page-60 row policy denominator drifted")
    rows = []
    for record in raw_rows:
        if not isinstance(record, dict):
            raise TMNoteWordBoxError("TM page-60 row policy is invalid")
        try:
            row_kind = TMNoteRowKind(str(record.get("row_kind")))
        except ValueError as exc:
            raise TMNoteWordBoxError("TM page-60 row kind is invalid") from exc
        ...
        if (
            ...
            raise TMNoteWordBoxError("TM page-60 row fields are invalid")
        rows.append(
            TMPage60RowSpec(
    ...
    if (
        ...
        raise TMNoteWordBoxError("TM page-60 metric/structural row order drifted")
    if sum(len(row.dash_axes) for row in rows) != 47:
        raise TMNoteWordBoxError("TM page-60 dash denominator drifted")

    dash_name = payload.get("dash_detector_config")
    if not isinstance(dash_name, str) or Path(dash_name).name != dash_name:
        raise TMNoteWordBoxError("TM page-60 dash-detector path is invalid")
    ...
    if not dash_path.is_file() or sha256_file(dash_path) != payload.get(
        ...
        raise TMNoteWordBoxError("TM page-60 dash detector is absent or drifted")
    forbidden = payload.get("forbidden_semantic_inputs")
    if not isinstance(forbidden, list) or set(forbidden) != _REQUIRED_FORBIDDEN:
        raise TMNoteWordBoxError("TM page-60 forbidden semantic inputs drifted")
    if (
        table.get("table_key") != "LIQUIDITY_RISK"
        or table.get("expected_structural_rows") != 2
        or table.get("expected_numeric_rows") != 20
        or continuation.get("page_60_closes_liquidity_table") is not True
        or continuation.get("page_61_starts_topic") != "EXCHANGE_RATE"
    ):
        raise TMNoteWordBoxError("TM page-60 table/continuation contract drifted")
    return TMPage60Policy(
        source_path=path,
        ...
        page_60_closes_liquidity_table=True,
        page_61_starts_topic="EXCHANGE_RATE",
...
def _find_anchor_line(
    ...
    *,
    ...
    if not candidates or candidates[0][0] < policy.minimum_anchor_similarity:
        raise TMNoteWordBoxError(f"TM page-60 anchor is unresolved: {anchors}")
...
def _find_anchor_group(
    ...
    *,
    ...
    if any(not group for group in choices):
        raise TMNoteWordBoxError(f"TM page-60 wrapped label is unresolved: {anchors}")
    candidates = []
    ...
    if not candidates:
        raise TMNoteWordBoxError("TM page-60 wrapped label lines are not locally adjacent")
...
def _axis_bindings(
    ...
    for ordinal, spec in enumerate(policy.axes, start=1):
        ...
        if abs(header[-1].x_right - spec.right_edge) > line_height * 3.0:
            raise TMNoteWordBoxError(f"TM page-60 header/grid edge drifted: {spec.axis_key}")
        results.append(
            TMPage60AxisBinding(
                ordinal=ordinal,
                ...
                unit_bbox=unit.bbox,
                evidence=(
                    "axis label matched only from the visible page-60 table header",
                    "right edge is the fixed visible seven-column numeric grid",
    ...
    *,
    ...
    return TMPage60LogicalRow(
        ...
        cell_period_starts=tuple(
            policy_date if numeric else None for policy_date in (axis.period_start for axis in axes)
        ),
        cell_period_ends=tuple(
            policy_date if numeric else None for policy_date in (axis.period_end for axis in axes)
        ),
...
def parse_tm_page60(
    ...
    policy: TMPage60Policy,
    *,
    page_tag: str = "page-0060",
) -> ParsedTMPage60:
    """Reconstruct the complete page-60 table with mapping authority disabled."""

    if page_tag != policy.page_tag:
        raise TMNoteWordBoxError("TM page-60 page tag drifted")
    if policy.canonical_unit != "VND" or policy.unit_multiplier != 1_000_000:
        raise TMNoteWordBoxError("TM page-60 native VND-million unit drifted")
    if sha256_file(result_path) != policy.source_ocr_sha256:
        raise TMNoteWordBoxError("TM page-60 compact OCR fixture hash drifted")
    if sha256_file(source_image_path) != policy.source_render_sha256:
        raise TMNoteWordBoxError("TM page-60 source render hash drifted")
    source_image = cv2.imread(str(source_image_path), cv2.IMREAD_GRAYSCALE)
    if source_image is None:
        raise TMNoteWordBoxError("TM page-60 source render cannot be decoded")
    ...
    if metadata != {
        ...
        raise TMNoteWordBoxError("TM page-60 compact OCR provenance drifted")
    line_height = float(statistics.median(line.height for line in lines))
    if line_height <= 0:
        raise TMNoteWordBoxError("TM page-60 line height is invalid")
    ...
    try:
        ...
        raise TMNoteWordBoxError("TM page-60 compact OCR scores are unreadable") from exc
    rejected_low_score_indices = tuple(
        index for index, score in enumerate(raw_scores) if float(score) < policy.minimum_line_score
    )
    if rejected_low_score_indices != _EXPECTED_REJECTED_LOW_SCORE_INDICES:
        raise TMNoteWordBoxError(
            f"TM page-60 rejected low-confidence line set drifted: {rejected_low_score_indices}"
        )
    ...
    if parse_vietnamese_date(title.text) != policy.visible_date:
        raise TMNoteWordBoxError("TM page-60 visible snapshot date drifted")
    ...
    if len(unit_candidates) != 1:
        raise TMNoteWordBoxError("TM page-60 visible unit denominator drifted")
    ...
    if tuple(min(line.y_center for line in group) for group in labels) != tuple(
        ...
        raise TMNoteWordBoxError("TM page-60 source row order drifted")
    axes = _axis_bindings(
        lines,
        title,
        super_header,
    ...
    if artifact_indices != _EXPECTED_ARTIFACT_INDICES:
        raise TMNoteWordBoxError(f"TM page-60 tiny OCR artifact set drifted: {artifact_indices}")
    ...
    if len(numeric_groups) != 19 or tuple(len(group) for group in numeric_groups) != (
        ...
        raise TMNoteWordBoxError(
            "TM page-60 numeric group denominator drifted: "
    ...
    for ordinal, (spec, label_lines) in enumerate(zip(policy.rows, labels, strict=True), start=1):
        if spec.row_kind is TMNoteRowKind.LABEL_ONLY:
            ...
            for line in group:
                ...
                if distances[axis_index] > line_height * 1.5 or per_axis[axis_index]:
                    raise TMNoteWordBoxError("TM page-60 numeric cell geometry drifted")
            ...
            if observed_value_axes != expected_value_axes:
                raise TMNoteWordBoxError(
                    f"TM page-60 row value/dash partition drifted: {spec.metric_key}"
                )
            ...
            for index, axis in enumerate(axes):
                ...
                if evidence is None:
                    raise TMNoteWordBoxError(
                        "TM page-60 visible dash lacks constrained pixel evidence: "
        ...
        if spec.row_kind is TMNoteRowKind.NUMERIC and any(
            ...
            raise TMNoteWordBoxError(
                f"TM page-60 row contains an unresolved cell: {spec.metric_key}"
            )
        rows.append(logical)
    if group_index != len(numeric_groups):
        raise TMNoteWordBoxError("TM page-60 numeric groups were not consumed exactly once")
    ...
    result = ParsedTMPage60(
        ...
        continues_to_page_61=not policy.page_60_closes_liquidity_table,
        next_page_topic=policy.page_61_starts_topic,
        mapping_authority=False,
        evidence=(
            "one complete page-60 liquidity-risk table reconstructed from immutable PP-OCRv6 geometry",
            "seven visible maturity axes use one fixed source-derived numeric grid",
            "20 numeric rows and two structural rows preserve source order and hierarchy",
            "93 finite values and 47 pixel-backed dashes preserve all 140 financial slots",
            "one low-confidence false glyph at a printed dash is rejected before row reconstruction",
            ...
            "page 60 closes the table; page 61 begins independently owned exchange-rate-risk content",
        ),
    )
    if (
        len(result.rows) != 22
        ...
        or result.financial_slot_count != 140
        or result.observation_count(ObservationKind.VALUE) != 93
        or result.observation_count(ObservationKind.ZERO) != 0
        or result.observation_count(ObservationKind.DASH) != 47
        ...
        or result.excluded_low_confidence_line_indices != _EXPECTED_REJECTED_LOW_SCORE_INDICES
        or result.excluded_footer_numeric_line_indices != (135,)
        or result.continues_to_page_61
        or result.next_page_topic != "EXCHANGE_RATE"
    ):
        raise TMNoteWordBoxError(
            "TM page-60 exact source denominator drifted: "
