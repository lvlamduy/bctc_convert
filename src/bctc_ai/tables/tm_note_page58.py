"""Hash-bound fixed-grid reconstruction for MBB consolidated TM page 58.

The page contains one currency-risk table with four converted-currency axes.
Printed dashes are retained only when constrained pixel evidence exists.  The two
section labels retain their eight structural blank positions as source evidence,
but those positions are excluded from the financial-slot denominator.
"""
...
_AXIS_RIGHT_EDGES = (1855.0, 2278.0, 2759.0, 3236.0)
...
_EXPECTED_NUMERIC_GROUP_SIZES = (4, 4, 4, 4, 2, 2, 3, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4)
...
class TMPage58Policy:
    ...
    axes: tuple[TMPage58AxisSpec, ...]
    rows: tuple[TMPage58RowSpec, ...]
    ...
    page_58_closes_currency_risk_table: bool
    page_59_starts_topic: str
    forbidden_semantic_inputs: tuple[str, ...]


@dataclass(frozen=True)
class TMPage58AxisBinding:
    ordinal: int
    axis_key: str
    canonical_label: str
...
class TMPage58Table:
    ...
    axes: tuple[TMPage58AxisBinding, ...]
    rows: tuple[TMPage58LogicalRow, ...]
    bbox: BoundingBox


@dataclass(frozen=True)
class ParsedTMPage58:
    input_path: str
    source_sha256: str
    upstream_ocr_sha256: str
    source_render_sha256: str
    ...
    @property
    def structural_blank_count(self) -> int:
        return sum(
...
def _anchors(value: Any, field: str) -> tuple[str, ...]:
    if not isinstance(value, list) or not value:
        raise TMNoteWordBoxError(f"TM page-58 {field} anchors are invalid")
    result = tuple(retrieval_key(str(item)) for item in value)
    if any(not item for item in result):
        raise TMNoteWordBoxError(f"TM page-58 {field} contains an empty anchor")
    return result


def _anchor_groups(value: Any, field: str) -> tuple[tuple[str, ...], ...]:
    if not isinstance(value, list) or not value:
        raise TMNoteWordBoxError(f"TM page-58 {field} groups are invalid")
...
def _date_value(payload: dict[str, Any], field: str) -> date:
    ...
    if isinstance(value, str):
        try:
            ...
            raise TMNoteWordBoxError(f"invalid TM page-58 {field}") from exc
    raise TMNoteWordBoxError(f"invalid TM page-58 {field}")


def _positive(payload: dict[str, Any], field: str, *, upper: float | None = None) -> float:
    value = payload.get(field)
    if isinstance(value, bool) or not isinstance(value, (int, float)) or value <= 0:
        raise TMNoteWordBoxError(f"invalid positive TM page-58 setting: {field}")
    result = float(value)
    if upper is not None and result >= upper:
        raise TMNoteWordBoxError(f"TM page-58 {field} must be below {upper}")
    return result


def load_tm_page58_policy(path: Path) -> TMPage58Policy:
    """Load the immutable source-scoped page-58 reconstruction contract."""

    try:
        payload = yaml.safe_load(path.read_text(encoding="utf-8"))
    except (OSError, yaml.YAMLError) as exc:
        raise TMNoteWordBoxError(f"cannot load TM page-58 policy: {path}") from exc
    if not isinstance(payload, dict) or (
        payload.get("version") != 1
        or payload.get("policy") != "TM_NOTE_PAGE58_CURRENCY_RISK_FIXED_GRID_DASH_V1"
        ...
        raise TMNoteWordBoxError("TM page-58 policy identity drifted")
    hash_fields = (
        "source_pdf_sha256",
    ...
    if any(not isinstance(value, str) or not _SHA256.fullmatch(value) for value in hashes):
        raise TMNoteWordBoxError("TM page-58 source hashes are invalid")
    thresholds = tuple(
        payload.get(field)
    ...
    if any(
        ...
        raise TMNoteWordBoxError("TM page-58 similarity thresholds are invalid")
    ...
    if not all(isinstance(item, dict) for item in (unit, reporting, table, geometry, continuation)):
        raise TMNoteWordBoxError("TM page-58 unit/period/table policy is incomplete")
    ...
    if (
        ...
        raise TMNoteWordBoxError("TM page-58 unit binding is invalid")
    if reporting.get("period_role") != "CURRENT" or reporting.get("period_type") != "SNAPSHOT":
        raise TMNoteWordBoxError("TM page-58 period role/type drifted")
    ...
    if (
        ...
        raise TMNoteWordBoxError("TM page-58 snapshot date drifted")

    raw_axes = payload.get("axes")
    if not isinstance(raw_axes, list) or len(raw_axes) != 4:
        raise TMNoteWordBoxError("TM page-58 must define four axes")
    axes = []
    for record in raw_axes:
        if not isinstance(record, dict):
            raise TMNoteWordBoxError("TM page-58 axis is invalid")
        right_edge = record.get("right_edge")
        if isinstance(right_edge, bool) or not isinstance(right_edge, (int, float)):
            raise TMNoteWordBoxError("TM page-58 axis edge is invalid")
        axes.append(
            TMPage58AxisSpec(
    ...
    if (
        ...
        raise TMNoteWordBoxError("TM page-58 axis order/geometry drifted")

    raw_rows = payload.get("rows")
    if not isinstance(raw_rows, list) or len(raw_rows) != 20:
        raise TMNoteWordBoxError("TM page-58 row policy denominator drifted")
    rows = []
    for record in raw_rows:
        if not isinstance(record, dict):
            raise TMNoteWordBoxError("TM page-58 row policy is invalid")
        try:
            row_kind = TMNoteRowKind(str(record.get("row_kind")))
        except ValueError as exc:
            raise TMNoteWordBoxError("TM page-58 row kind is invalid") from exc
        ...
        if (
            ...
            raise TMNoteWordBoxError("TM page-58 row fields are invalid")
        rows.append(
            TMPage58RowSpec(
    ...
    if (
        ...
        raise TMNoteWordBoxError("TM page-58 metric/structural row order drifted")
    if sum(len(row.dash_axes) for row in rows) != 9:
        raise TMNoteWordBoxError("TM page-58 dash denominator drifted")

    dash_name = payload.get("dash_detector_config")
    if not isinstance(dash_name, str) or Path(dash_name).name != dash_name:
        raise TMNoteWordBoxError("TM page-58 dash-detector path is invalid")
    ...
    if not dash_path.is_file() or sha256_file(dash_path) != payload.get(
        ...
        raise TMNoteWordBoxError("TM page-58 dash detector is absent or drifted")
    forbidden = payload.get("forbidden_semantic_inputs")
    if not isinstance(forbidden, list) or set(forbidden) != _REQUIRED_FORBIDDEN:
        raise TMNoteWordBoxError("TM page-58 forbidden semantic inputs drifted")
    if (
        table.get("table_key") != "CURRENCY_RISK"
        or table.get("expected_structural_rows") != 2
        or table.get("expected_numeric_rows") != 18
        or continuation.get("page_58_closes_currency_risk_table") is not True
        or continuation.get("page_59_starts_topic") != "LIQUIDITY_RISK"
    ):
        raise TMNoteWordBoxError("TM page-58 table/continuation contract drifted")
    return TMPage58Policy(
        source_path=path,
        ...
        page_58_closes_currency_risk_table=True,
        page_59_starts_topic="LIQUIDITY_RISK",
...
def _find_anchor_line(
    ...
    *,
    ...
    if not candidates or candidates[0][0] < policy.minimum_anchor_similarity:
        raise TMNoteWordBoxError(f"TM page-58 anchor is unresolved: {anchors}")
...
def _find_anchor_group(
    ...
    *,
    ...
    if any(not group for group in choices):
        raise TMNoteWordBoxError(f"TM page-58 wrapped label is unresolved: {anchors}")
    candidates = []
    ...
    if not candidates:
        raise TMNoteWordBoxError("TM page-58 wrapped label lines are not locally adjacent")
...
def _axis_bindings(
    ...
    for ordinal, spec in enumerate(policy.axes, start=1):
        ...
        if abs(header[-1].x_right - spec.right_edge) > line_height * 3.0:
            raise TMNoteWordBoxError(f"TM page-58 header/grid edge drifted: {spec.axis_key}")
        results.append(
            TMPage58AxisBinding(
                ordinal=ordinal,
                ...
                unit_bbox=unit.bbox,
                evidence=(
                    "axis label matched only from the visible page-58 table header",
                    "right edge is the fixed visible four-column numeric grid",
                    ...
                    f"currency-risk section title source line {section_title.index}",
                ),
            )
        )
...
def parse_tm_page58(
    ...
    policy: TMPage58Policy,
    *,
    page_tag: str = "page-0058",
) -> ParsedTMPage58:
    """Reconstruct the complete page-58 table with mapping authority disabled."""

    if page_tag != policy.page_tag:
        raise TMNoteWordBoxError("TM page-58 page tag drifted")
    if sha256_file(result_path) != policy.source_ocr_sha256:
        raise TMNoteWordBoxError("TM page-58 compact OCR fixture hash drifted")
    if sha256_file(source_image_path) != policy.source_render_sha256:
        raise TMNoteWordBoxError("TM page-58 source render hash drifted")
    source_image = cv2.imread(str(source_image_path), cv2.IMREAD_GRAYSCALE)
    if source_image is None:
        raise TMNoteWordBoxError("TM page-58 source render cannot be decoded")
    ...
    if metadata != {
        ...
        raise TMNoteWordBoxError("TM page-58 compact OCR provenance drifted")
    line_height = float(statistics.median(line.height for line in lines))
    if line_height <= 0:
        raise TMNoteWordBoxError("TM page-58 line height is invalid")
    source_bbox = _union(lines)
    footer_top = source_bbox.y1 * policy.page_footer_top_ratio

    section_title = _find_anchor_line(
        lines,
        policy.section_title_anchors,
        policy,
        maximum_y=footer_top,
    )
    title = _find_anchor_line(
        lines,
        policy.title_anchors,
        policy,
        minimum_y=section_title.y_center,
        maximum_y=footer_top,
    )
    if parse_vietnamese_date(title.text) != policy.visible_date:
        raise TMNoteWordBoxError("TM page-58 visible snapshot date drifted")
    unit_candidates = tuple(
        line
        for line in lines
        if title.y_center < line.y_center < 900
        and _best_anchor_similarity(line.text, policy.unit_anchors)
        >= policy.minimum_unit_similarity
    )
    if len(unit_candidates) != 1:
        raise TMNoteWordBoxError("TM page-58 visible unit denominator drifted")
    unit = unit_candidates[0]

    labels = []
    minimum_y = unit.y_center
    ...
    if tuple(min(line.y_center for line in group) for group in labels) != tuple(
        ...
        raise TMNoteWordBoxError("TM page-58 source row order drifted")
    axes = _axis_bindings(
        lines,
        title,
        section_title,
    ...
    if artifact_indices != _EXPECTED_ARTIFACT_INDICES:
        raise TMNoteWordBoxError(f"TM page-58 tiny OCR artifact set drifted: {artifact_indices}")
    ...
    if len(numeric_groups) != 17 or tuple(len(group) for group in numeric_groups) != (
        _EXPECTED_NUMERIC_GROUP_SIZES
    ):
        raise TMNoteWordBoxError(
            "TM page-58 numeric group denominator drifted: "
    ...
    for ordinal, (spec, label_lines) in enumerate(zip(policy.rows, labels, strict=True), start=1):
        if spec.row_kind is TMNoteRowKind.LABEL_ONLY:
            ...
            for line in group:
                ...
                if distances[axis_index] > line_height * 1.5 or per_axis[axis_index]:
                    raise TMNoteWordBoxError("TM page-58 numeric cell geometry drifted")
            ...
            if observed_value_axes != expected_value_axes:
                raise TMNoteWordBoxError(
                    f"TM page-58 row value/dash partition drifted: {spec.metric_key}"
                )
            ...
            for index, axis in enumerate(axes):
                ...
                if evidence is None:
                    raise TMNoteWordBoxError(
                        "TM page-58 visible dash lacks constrained pixel evidence: "
        ...
        if spec.row_kind is TMNoteRowKind.NUMERIC and any(
            ...
            raise TMNoteWordBoxError(
                f"TM page-58 row contains an unresolved cell: {spec.metric_key}"
            )
        rows.append(logical)
    if group_index != len(numeric_groups):
        raise TMNoteWordBoxError("TM page-58 numeric groups were not consumed exactly once")
    ...
    result = ParsedTMPage58(
        ...
        continues_to_page_59=not policy.page_58_closes_currency_risk_table,
        next_page_topic=policy.page_59_starts_topic,
        mapping_authority=False,
        evidence=(
            "one complete page-58 currency-risk table reconstructed from immutable PP-OCRv6 geometry",
            "four visible converted-currency axes use one fixed source-derived numeric grid",
            "18 numeric rows and two structural rows preserve source order and hierarchy",
            "63 finite values and nine pixel-backed dashes preserve all 72 financial slots",
            "eight structural blanks are retained outside the financial-slot denominator",
            ...
            "page 58 closes the currency table; page 59 begins independently owned liquidity-risk content",
        ),
    )
    if (
        len(result.rows) != 20
        or result.numeric_row_count != 18
        or result.label_only_row_count != 2
        or result.financial_slot_count != 72
        or result.observation_count(ObservationKind.VALUE) != 63
        ...
        or result.structural_blank_count != 8
        or result.unassigned_numeric_line_indices
        or result.excluded_artifact_line_indices != _EXPECTED_ARTIFACT_INDICES
        or result.excluded_footer_numeric_line_indices != (99,)
        or result.continues_to_page_59
        or result.next_page_topic != "LIQUIDITY_RISK"
    ):
        raise TMNoteWordBoxError(
            "TM page-58 exact source denominator drifted: "
