"""Hash-bound dual-duration TM reconstruction for MBB consolidated PDF page 46."""
...
@dataclass(frozen=True)
class TMPage46StructuralSpec:
    source_role: str
    anchors: tuple[str, ...]


@dataclass(frozen=True)
class TMPage46TableSpec:
    table_key: str
    note_number: str
    title_anchors: tuple[str, ...]
    body_end_anchors: tuple[str, ...]
    section_title_anchors: tuple[str, ...]
    structural_rows: tuple[TMPage46StructuralSpec, ...]
...
class TMPage46Policy:
    ...
    expected_axes: tuple[TMPage46ExpectedAxis, ...]
    ...
    tables: tuple[TMPage46TableSpec, ...]
    forbidden_semantic_inputs: tuple[str, ...]


@dataclass(frozen=True)
class TMPage46AxisBinding:
    ordinal: int
    axis_id: str
...
class TMPage46Table:
    ...
    axes: tuple[TMPage46AxisBinding, ...]
    rows: tuple[TMPage46LogicalRow, ...]
    bbox: BoundingBox


@dataclass(frozen=True)
class ParsedTMPage46:
    input_path: str
    source_sha256: str
    source_render_sha256: str
    source_pdf_sha256: str
...
def _anchors(payload: Any, field: str, *, allow_empty: bool = False) -> tuple[str, ...]:
    if not isinstance(payload, list) or (not payload and not allow_empty):
        raise TMNoteWordBoxError(f"TM page-46 {field} anchors are invalid")
    result = tuple(retrieval_key(str(value)) for value in payload)
    if any(not value for value in result):
        raise TMNoteWordBoxError(f"TM page-46 {field} contains an empty anchor")
    return result
...
def _positive(payload: dict[str, Any], field: str) -> float:
    ...
    if isinstance(value, bool) or not isinstance(value, (int, float)) or value <= 0:
        raise TMNoteWordBoxError(f"invalid positive TM page-46 setting: {field}")
    return float(value)
...
def _date_value(value: Any, field: str) -> date:
    ...
    if isinstance(value, str):
        try:
            ...
            raise TMNoteWordBoxError(f"invalid TM page-46 {field}") from exc
    raise TMNoteWordBoxError(f"invalid TM page-46 {field}")


def load_tm_page46_policy(path: Path) -> TMPage46Policy:
    try:
        payload = yaml.safe_load(path.read_text(encoding="utf-8"))
    except (OSError, yaml.YAMLError) as exc:
        raise TMNoteWordBoxError(f"cannot load TM page-46 policy: {path}") from exc
    if not isinstance(payload, dict) or (
        payload.get("version") != 1
        or payload.get("policy") != "TM_NOTE_PAGE46_DUAL_DURATION_FIXED_GRID_DASH_V1"
        ...
        raise TMNoteWordBoxError("TM page-46 policy identity drifted")
    hashes = (
    ...
    if any(not isinstance(value, str) or not _SHA256.fullmatch(value) for value in hashes):
        raise TMNoteWordBoxError("TM page-46 source hashes are invalid")
    thresholds = (
    ...
    if any(
        ...
        raise TMNoteWordBoxError("TM page-46 similarity thresholds are invalid")
    ...
    if not all(isinstance(value, dict) for value in (unit, header, geometry)):
        raise TMNoteWordBoxError("TM page-46 geometry policy is incomplete")
    ...
    if (
        ...
        raise TMNoteWordBoxError("TM page-46 unit binding is invalid")
    raw_axes = header.get("expected_axes")
    if not isinstance(raw_axes, list) or len(raw_axes) != 2:
        raise TMNoteWordBoxError("TM page-46 must define two duration axes")
    expected_axes = []
    for record in raw_axes:
        if not isinstance(record, dict) or record.get("role") not in {
            ...
            raise TMNoteWordBoxError("TM page-46 duration-axis record is invalid")
        expected_axes.append(
            TMPage46ExpectedAxis(
    ...
    if tuple(axis.role for axis in expected_axes) != ("CURRENT", "COMPARATIVE"):
        raise TMNoteWordBoxError("TM page-46 duration-axis order drifted")
    dash_name = payload.get("dash_detector_config")
    if not isinstance(dash_name, str) or Path(dash_name).name != dash_name:
        raise TMNoteWordBoxError("TM page-46 dash detector path is invalid")
    ...
    if not dash_path.is_file() or sha256_file(dash_path) != payload.get(
        ...
        raise TMNoteWordBoxError("TM page-46 dash detector is absent or drifted")
    raw_tables = payload.get("tables")
    if not isinstance(raw_tables, list) or len(raw_tables) != 2:
        raise TMNoteWordBoxError("TM page-46 must define two visible notes")
    tables = []
    for record in raw_tables:
        if not isinstance(record, dict):
            raise TMNoteWordBoxError("TM page-46 table record is invalid")
        raw_structural = record.get("structural_rows")
        if not isinstance(raw_structural, list) or len(raw_structural) != 2:
            raise TMNoteWordBoxError("TM page-46 table must define two section headings")
        structural = []
        for item in raw_structural:
            if not isinstance(item, dict) or not isinstance(item.get("source_role"), str):
                raise TMNoteWordBoxError("TM page-46 structural row is invalid")
            structural.append(
                TMPage46StructuralSpec(
                    source_role=item["source_role"],
                    anchors=_anchors(item.get("anchors"), "structural row"),
                )
            )
        ...
        if (
            ...
            raise TMNoteWordBoxError("TM page-46 table identity or denominator is invalid")
        tables.append(
            TMPage46TableSpec(
                table_key=table_key,
    ...
    if [table.table_key for table in tables] != ["NET_INTEREST", "NET_SERVICE"]:
        raise TMNoteWordBoxError("TM page-46 note order drifted")
    forbidden = payload.get("forbidden_semantic_inputs")
    if not isinstance(forbidden, list) or set(forbidden) != _REQUIRED_FORBIDDEN:
        raise TMNoteWordBoxError("TM page-46 forbidden semantic inputs drifted")
    ...
    if footer_ratio >= 1 or artifact_ratio >= 1:
        raise TMNoteWordBoxError("TM page-46 page ratios must be below one")
    dash_config = load_word_box_reconstruction_v3_config(dash_path).base.base
    return TMPage46Policy(
        source_path=path,
...
def _find_anchor_line(
    ...
    *,
    ...
    if not candidates or candidates[0][0] < policy.minimum_anchor_similarity:
        raise TMNoteWordBoxError(f"TM page-46 anchor is unresolved: {anchors}")
...
def _number_line(lines: tuple[_Line, ...], number: str, title: _Line, line_height: float) -> _Line:
    ...
    if len(candidates) != 1:
        raise TMNoteWordBoxError(f"TM page-46 note {number} number anchor drifted")
    return candidates[0]
...
def _duration_axes(
    ...
    if len(date_lines) != 4 or len(unit_lines) != 2:
        raise TMNoteWordBoxError("TM page-46 duration header must expose four dates and two units")
    ...
    if (
        ...
        raise TMNoteWordBoxError("TM page-46 duration axes are not distinctly separated")
    ...
    for date_line in date_lines:
        ...
        if (
            ...
            raise TMNoteWordBoxError("TM page-46 date/unit geometry is not locally bounded")
        assigned[unit_line.index].append(date_line)
    if any(len(group) != 2 for group in assigned.values()):
        raise TMNoteWordBoxError("TM page-46 duration date pairing drifted")
    result = []
    for ordinal, (unit_line, expected) in enumerate(
        ...
        if any(value is None for value in parsed_dates):
            raise TMNoteWordBoxError("TM page-46 duration date parsing failed")
        ...
        if (visible_start, visible_end) != (expected.period_start, expected.period_end):
            raise TMNoteWordBoxError("TM page-46 visible duration drifted from policy")
...
def _renumber(
    rows: list[TMPage46LogicalRow], *, page_tag: str, table_key: str
) -> tuple[TMPage46LogicalRow, ...]:
    result = []
...
def _reconstruct_table(
    lines: tuple[_Line, ...],
    spec: TMPage46TableSpec,
    title: _Line,
    axes: tuple[TMPage46AxisBinding, ...],
    headings: tuple[tuple[TMPage46StructuralSpec, _Line], ...],
    body_end: float,
    policy: TMPage46Policy,
    line_height: float,
    source_image: Any,
    ...
    *,
...
) -> tuple[TMPage46Table, tuple[int, ...], tuple[int, ...]]:
    ...
    if not numeric_groups or any(len(group) != 2 for group in numeric_groups):
        raise TMNoteWordBoxError(f"TM page-46 {spec.table_key} has incomplete numeric rows")
    numeric_centers = [
    ...
    used.update(line.index for _record, line in headings)
    ...
    for label in dash_labels:
        ...
        if any(record is None for record in evidence):
            raise TMNoteWordBoxError(
                f"TM page-46 visible dash pair lacks constrained pixel evidence: {spec.table_key}"
            )
    ...
    if len(centers) != spec.expected_numeric_rows:
        raise TMNoteWordBoxError(f"TM page-46 {spec.table_key} numeric denominator drifted")
    ...
    for center_index, center in enumerate(centers):
        ...
        if any(
            ...
            raise TMNoteWordBoxError(f"TM page-46 {spec.table_key} has an unresolved cell")
    ...
    for structural_spec, line in headings:
        rows.append(
            _structural_row(
                page_tag=page_tag,
                table_key=spec.table_key,
    ...
    if counts != (spec.expected_numeric_rows, spec.expected_label_only_rows):
        raise TMNoteWordBoxError(
            f"TM page-46 {spec.table_key} denominator drifted: {counts[0]} numeric + "
...
def parse_tm_page46(
    ...
    policy: TMPage46Policy,
    *,
    page_tag: str = "page-0046",
) -> ParsedTMPage46:
    if page_tag != policy.page_tag:
        raise TMNoteWordBoxError("TM page-46 page tag drifted")
    if sha256_file(result_path) != policy.source_ocr_sha256:
        raise TMNoteWordBoxError("TM page-46 OCR artifact hash drifted")
    if sha256_file(source_image_path) != policy.source_render_sha256:
        raise TMNoteWordBoxError("TM page-46 source render hash drifted")
    source_image = cv2.imread(str(source_image_path), cv2.IMREAD_GRAYSCALE)
    if source_image is None:
        raise TMNoteWordBoxError("TM page-46 source render cannot be decoded")
    ...
    if line_height <= 0:
        raise TMNoteWordBoxError("TM page-46 line height is invalid")
    ...
    if tuple(title.y_center for title in titles) != tuple(
        ...
        raise TMNoteWordBoxError("TM page-46 note-title order drifted")
    tables = []
    unassigned = []
    artifacts = []
    for ordinal, (spec, title) in enumerate(zip(policy.tables, titles, strict=True), start=1):
        ...
        if tuple(line.y_center for _record, line in headings) != tuple(
            sorted(line.y_center for _record, line in headings)
        ):
            raise TMNoteWordBoxError(f"TM page-46 {spec.table_key} heading order drifted")
        axes = _duration_axes(lines, title, headings[0][1], policy, line_height)
        section_title = (
            _find_anchor_line(
                lines,
    ...
    if any(
        ...
        raise TMNoteWordBoxError("TM page-46 repeated duration headers disagree semantically")
    ...
    result = ParsedTMPage46(
        ...
        evidence=(
            "two complete source notes bounded on immutable PDF page 46",
            "two repeated duration axes bound independently from visible dates and units",
            "31 numeric rows reconstructed from PP-OCRv6 geometry plus pixel-backed DASH",
            "schema identifiers are absent from parser inputs and mapping authority remains false",
        ),
    )
    if (
        len(result.rows) != 38
        or result.numeric_row_count != 31
        or result.label_only_row_count != 7
        or result.financial_slot_count != 62
        or result.observation_count(ObservationKind.VALUE) != 60
        or result.observation_count(ObservationKind.DASH) != 2
        or result.unassigned_numeric_line_indices
        or result.excluded_artifact_line_indices != (77, 93)
        or result.excluded_footer_numeric_line_indices != (113,)
    ):
        raise TMNoteWordBoxError(
            "TM page-46 exact source denominator drifted: "
