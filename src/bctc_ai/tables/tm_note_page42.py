"""Fixed-source TM table reconstruction for MBB consolidated PDF page 42."""
...
TMPage42Policy = TMPage35Policy
TMPage42LogicalRow = TMPage35LogicalRow
TMPage42Table = TMPage35Table


@dataclass(frozen=True)
class ParsedTMPage42(ParsedTMPage35):
    """Page-42 result using the shared fixed-grid row/table contracts."""
...
def _anchors(payload: Any, field: str, *, allow_empty: bool = False) -> tuple[str, ...]:
    if not isinstance(payload, list) or (not payload and not allow_empty):
        raise TMNoteWordBoxError(f"TM page-42 {field} anchors are invalid")
    result = tuple(retrieval_key(str(value)) for value in payload)
    if any(not value for value in result):
        raise TMNoteWordBoxError(f"TM page-42 {field} contains an empty anchor")
    return result
...
def _positive(payload: dict[str, Any], field: str) -> float:
    ...
    if isinstance(value, bool) or not isinstance(value, (int, float)) or value <= 0:
        raise TMNoteWordBoxError(f"invalid positive TM page-42 setting: {field}")
    return float(value)


def load_tm_page42_policy(path: Path) -> TMPage42Policy:
    """Load the immutable, source-scoped page-42 reconstruction policy."""

    try:
        payload = yaml.safe_load(path.read_text(encoding="utf-8"))
    except (OSError, yaml.YAMLError) as exc:
        raise TMNoteWordBoxError(f"cannot load TM page-42 policy: {path}") from exc
    if not isinstance(payload, dict) or (
        payload.get("version") != 1
        or payload.get("policy") != "TM_NOTE_PAGE42_FIXED_GRID_V1"
        ...
        raise TMNoteWordBoxError("TM page-42 policy identity drifted")
    hashes = (
    ...
    if any(not isinstance(value, str) or not _SHA256.fullmatch(value) for value in hashes):
        raise TMNoteWordBoxError("TM page-42 source hashes are invalid")
    thresholds = (
    ...
    if any(
        ...
        raise TMNoteWordBoxError("TM page-42 similarity thresholds are invalid")
    ...
    if not all(isinstance(value, dict) for value in (unit, header, geometry)):
        raise TMNoteWordBoxError("TM page-42 geometry policy is incomplete")
    ...
    if (
        ...
        raise TMNoteWordBoxError("TM page-42 unit binding is invalid")
    dash_name = payload.get("dash_detector_config")
    if not isinstance(dash_name, str) or Path(dash_name).name != dash_name:
        raise TMNoteWordBoxError("TM page-42 dash detector path is invalid")
    ...
    if not dash_path.is_file() or sha256_file(dash_path) != payload.get(
        ...
        raise TMNoteWordBoxError("TM page-42 dash detector is absent or drifted")
    dash_config = load_word_box_reconstruction_v3_config(dash_path).base.base
    raw_tables = payload.get("tables")
    if not isinstance(raw_tables, list) or len(raw_tables) != 5:
        raise TMNoteWordBoxError("TM page-42 must define five visible tables")
    tables = []
    for record in raw_tables:
        if not isinstance(record, dict):
            raise TMNoteWordBoxError("TM page-42 table record is invalid")
        ...
        if (
            ...
            raise TMNoteWordBoxError("TM page-42 table denominator is invalid")
        tables.append(
            TMPage35TableSpec(
                table_key=table_key,
    ...
    if len({table.table_key for table in tables}) != len(tables):
        raise TMNoteWordBoxError("TM page-42 table keys are duplicated")
    forbidden = payload.get("forbidden_semantic_inputs")
    if not isinstance(forbidden, list) or set(forbidden) != _REQUIRED_FORBIDDEN:
        raise TMNoteWordBoxError("TM page-42 forbidden semantic inputs drifted")
    ...
    if (
        ...
        raise TMNoteWordBoxError("TM page-42 structural-continuation bound is invalid")
    if footer_ratio >= 1:
        raise TMNoteWordBoxError("TM page-42 footer ratio must be below one")
...
def parse_tm_page42(
    ...
    policy: TMPage42Policy,
    *,
    page_tag: str = "page-0042",
) -> ParsedTMPage42:
    """Reconstruct all visible page-42 rows without granting mapping authority."""

    if page_tag != policy.page_tag:
        raise TMNoteWordBoxError("TM page-42 page tag drifted")
    if sha256_file(result_path) != policy.source_ocr_sha256:
        raise TMNoteWordBoxError("TM page-42 OCR artifact hash drifted")
    if sha256_file(source_image_path) != policy.source_render_sha256:
        raise TMNoteWordBoxError("TM page-42 source render hash drifted")
    source_image = cv2.imread(str(source_image_path), cv2.IMREAD_GRAYSCALE)
    if source_image is None:
        raise TMNoteWordBoxError("TM page-42 source render cannot be decoded")
    ...
    if line_height <= 0:
        raise TMNoteWordBoxError("TM page-42 line height is invalid")
    ...
    if any(
        ...
        raise TMNoteWordBoxError("TM page-42 repeated headers disagree semantically")
    ...
    result = ParsedTMPage42(
        ...
        evidence=(
            "five local tables located from visible page-42 titles in source order",
            "two snapshot axes bound independently from every visible local header",
            "48 values reconstructed from PP-OCRv6 geometry",
    ...
    if (
        ...
        or result.label_only_row_count != 0
        or result.financial_slot_count != 48
        or result.observation_count(ObservationKind.VALUE) != 48
        or result.observation_count(ObservationKind.DASH) != 0
    ):
        raise TMNoteWordBoxError("TM page-42 page denominator drifted")
    return result
