...
def build_table_axis_repair_prompt_v1(
    *, base_page_json_version_id: str, targets: Sequence[dict[str, Any]]
    ...
    return (
        ...
        "Phải chép toàn bộ chuỗi tiêu đề gắn với bảng: nếu ảnh có cả dòng owner 'BÁO CÁO "
        "BỘ PHẬN' và dòng tiểu mục thì giữ cả hai theo thứ tự, không rút gọn chỉ còn dòng "
        "tiểu mục. Nếu không đọc chắc toàn bộ tiêu đề thì ghi uncertainty_exact thay vì bỏ "
        "một phần. "
