...
__all__ = [
    "AuthenticatedFamilyFirstPPocrV6NumericIndexV2",
    "FamilyFirstPPocrV6NumericIndexV2Error",
    ...
    "read_authenticated_family_first_ppocrv6_numeric_document_v2",
    "read_authenticated_family_first_ppocrv6_numeric_evidence_batch_v2",
]


INDEX_ROOT = runner_v2.CACHE_ROOT / "verified-index"
RECEIPT_PATH = INDEX_ROOT / "verification-receipt.json"
FORMAT_VERSION = "FAMILY_FIRST_PPOCRV6_NUMERIC_INDEX_RECEIPT_V2"
...
class FamilyFirstPPocrV6NumericIndexV2Error(RuntimeError):
    """The aggregate, source join, selected crop, or live receipt drifted."""


def _error(message: str) -> FamilyFirstPPocrV6NumericIndexV2Error:
    return FamilyFirstPPocrV6NumericIndexV2Error(message)
...
def _axis(proposal_payload: bytes, sample_count: int) -> str:
    ...
    for _start, _stop, proposal in runner_v2._proposal_iterator(
        proposal_payload,
...
def _receipt(
    ...
    material = {
        ...
        "aggregate_ref": {
            "path": (runner_v2.AGGREGATE_ROOT / runner_v2._AGGREGATE_MANIFEST_NAME).as_posix(),
    ...
    return _validate_receipt(
        {**material, "receipt_id": "ffpniv2:receipt:" + canonical_json_sha256_v1(material)}
    )
...
def _validate_receipt(value: Any) -> dict[str, Any]:
    ...
    if (
        ...
        or not value["aggregate_id"].startswith("ffpnav2:aggregate:")
        ...
        raise _error("numeric V2 index receipt contract drifted")
    try:
        archive_v1._ref(value["aggregate_ref"], "numeric V2 aggregate receipt ref")
        archive_v1._ref(value["proposal_ref"], "numeric V2 proposal receipt ref")
    except archive_v1.FamilyFirstSemanticLabelArchiveV1Error as exc:
        raise _error("numeric V2 index artifact reference drifted") from exc
    material = canonical_clone_v1(value)
    receipt_id = material.pop("receipt_id")
    if receipt_id != "ffpniv2:receipt:" + canonical_json_sha256_v1(material):
        raise _error("numeric V2 index receipt identity drifted")
    return canonical_clone_v1(value)


def _publish_receipt(root: Path, payload: bytes) -> None:
    cache_fd, shards_fd = runner_v2._cache_fds(root)
    ...
    try:
        try:
            ...
            raise _error("fixed numeric V2 verified index already exists")
        ...
        if stage_identity is None or (named.st_dev, named.st_ino) != stage_identity:
            raise _error("published numeric V2 index inode differs from its completed stage")
        stage_name = ""
    finally:
...
def finalize_authenticated_family_first_ppocrv6_numeric_index_v2(
    project_root: Path,
    ...
    *,
    ...
    aggregate, aggregate_payload, proposal_payload, _offsets = (
        runner_v2.validate_authenticated_family_first_ppocrv6_numeric_aggregate_v2(
...
class AuthenticatedFamilyFirstPPocrV6NumericIndexV2:
    ...
    def __new__(cls, token: object | None = None):
        if token is not _MINT:
            raise TypeError("family-first numeric V2 index handles cannot be constructed")
        return super().__new__(cls)

    def __copy__(self) -> None:
        raise TypeError("family-first numeric V2 index handles cannot be copied")

    def __deepcopy__(self, _memo: dict[int, Any]) -> None:
        raise TypeError("family-first numeric V2 index handles cannot be copied")

    def __reduce__(self) -> None:
        raise TypeError("family-first numeric V2 index handles cannot be pickled")


_MINT = object()
_INDICES: weakref.WeakKeyDictionary[AuthenticatedFamilyFirstPPocrV6NumericIndexV2, _IndexState] = (
    weakref.WeakKeyDictionary()
)


def authenticate_family_first_ppocrv6_numeric_index_v2(
    project_root: Path,
    archive_capability: archive_v1.AuthenticatedFamilyFirstSemanticLabelArchiveV1,
    *,
    model_cache: Path,
    ...
    receipt_payload = _root_bytes(root, RECEIPT_PATH, "numeric V2 index receipt")
    persisted = _validate_receipt(_canonical_object(receipt_payload, "numeric V2 index receipt"))
    expected = _receipt(
        archive_manifest,
        batch,
        plan,
    ...
    if not same_typed_json_v1(persisted, expected):
        raise _error("numeric V2 index receipt does not replay exactly")
    capability = AuthenticatedFamilyFirstPPocrV6NumericIndexV2(_MINT)
...
def _live_index(
    ...
    if type(capability) is not AuthenticatedFamilyFirstPPocrV6NumericIndexV2:
        raise _error("one exact live family-first numeric V2 index handle is required")
    state = _INDICES.get(capability)
    if state is None:
        raise _error("family-first numeric V2 index handle is not live")
    archive_state, _archive_manifest, batch, plan, private = archive_v1._archive_payloads(
        state.archive
    )
    receipt_payload = _root_bytes(state.root, RECEIPT_PATH, "numeric V2 index receipt")
    aggregate_payload = _root_bytes(
        state.root,
        runner_v2.AGGREGATE_ROOT / runner_v2._AGGREGATE_MANIFEST_NAME,
        "numeric V2 aggregate manifest",
    )
    proposal_payload = _root_bytes(
        state.root,
        runner_v2.AGGREGATE_ROOT / runner_v2._PROPOSAL_NAME,
        "numeric V2 aggregate proposals",
    )
    if (
        ...
        raise _error("numeric V2 live artifacts changed after authentication")
    receipt = _validate_receipt(_canonical_object(receipt_payload, "numeric V2 receipt"))
    aggregate = _canonical_object(aggregate_payload, "numeric V2 aggregate")
    runner_v2._git_ledger(state.root, aggregate.get("git_binding"))
    config_payload, config_ref = runner_v2._configuration_ref(state.root)
    ...
    if (
        ...
        raise _error("numeric V2 live config/model lineage drifted")
    return state, receipt, batch, plan, private


def project_authenticated_family_first_ppocrv6_numeric_index_v2(
    capability: AuthenticatedFamilyFirstPPocrV6NumericIndexV2,
) -> dict[str, Any]:
...
def _proposal_at(state: _IndexState, ordinal: int) -> dict[str, Any]:
    ...
    try:
        ...
        raise _error("numeric V2 proposal snapshot drifted") from exc
...
def _document_range(private: dict[str, Any], document_ordinal: int) -> tuple[int, int]:
    ...
    if not starts:
        raise _error("numeric V2 source document retained no samples")
    return starts[0], starts[-1] + 1


def read_authenticated_family_first_ppocrv6_numeric_document_v2(
    capability: AuthenticatedFamilyFirstPPocrV6NumericIndexV2,
    *,
    ...
    if (
        ...
        raise _error("numeric V2 document ordinal lies outside the corpus")
...
def _selections(value: Any) -> tuple[tuple[int, int, int], ...]:
    if type(value) is not tuple or not value:
        raise _error("numeric V2 evidence selection must be one non-empty exact tuple")
    result = []
    for raw in value:
        if (
            ...
            raise _error("numeric V2 evidence selection locator drifted")
        result.append((raw["document_ordinal"], raw["physical_page"], raw["line_ordinal"]))
    if result != sorted(set(result)):
        raise _error("numeric V2 evidence selections must be unique and source-ordered")
    return tuple(result)


def read_authenticated_family_first_ppocrv6_numeric_evidence_batch_v2(
    capability: AuthenticatedFamilyFirstPPocrV6NumericIndexV2,
    *,
    ...
    try:
        ...
        raise _error("numeric V2 evidence selection does not identify one source line") from exc
    ...
    while cursor <= final_target:
        ...
        if not chunk:
            raise _error("authenticated crop stream ended before numeric V2 selections")
        for crop in chunk:
            if cursor in target_indices:
                ...
                if (
                    ...
                    raise _error("selected numeric V2 crop/source/proposal cross-link drifted")
                provider = {
                    "input_path": None,
                    "page_index": None,
    ...
    if set(built) != set(locators):
        raise _error("numeric V2 evidence batch did not retain every selection")
