"""Family-32 adapter for trading-securities activity disclosures.
...
ADAPTER_FORMAT_VERSION = "GEMINI_JSON_TRADING_SECURITIES_ACTIVITY_FAMILY_ADAPTER_V1"
...
PRIMARY_ROOT_QUERY_RECEIPT_FORMAT_VERSION = (
    "GEMINI_JSON_TRADING_SECURITIES_ACTIVITY_PRIMARY_ROOT_QUERY_RECEIPT_V1"
)
DEFAULT_SOURCE_REPAIR_PATH = (
    "data/registered/gemini_json_trading_securities_activity_source_repairs_v1.json"
)
CLAIM_BOUNDARY = (
    "MANIFEST_SELECTED_GEMINI_JSON_ONLY_FAMILY32_EXACT_PDF_TOKEN_REPAIR_"
...
class GeminiJsonTradingSecuritiesActivityFamilyV1Error(ValueError):
    """Family-32 adapter input, source evidence, or replay drifted."""


def _error(message: str) -> GeminiJsonTradingSecuritiesActivityFamilyV1Error:
    return GeminiJsonTradingSecuritiesActivityFamilyV1Error(message)
...
def _load_default_source_repair_artifact_v1() -> dict[str, Any]:
    ...
    try:
        ...
        raise _error("trading-securities source-repair artifact is absent or invalid") from exc
...
def _compile_source_repair_artifact_v1(value: Any) -> dict[str, Any]:
    ...
    if (
        ...
        raise _error("trading-securities source-repair artifact is invalid")
    repair_fields = {
    ...
    for raw in value["repairs"]:
        if type(raw) is not dict or set(raw) != repair_fields:
            raise _error("trading-securities source-repair fields drifted")
        ...
        if (
            ...
            raise _error("trading-securities source-repair binding is invalid")
        ...
        for cell in repair["cell_repairs"]:
            ...
            if (
                ...
                raise _error("trading-securities source-repair cell is invalid")
        ...
        for column in repair["column_repairs"]:
            if (
                ...
                raise _error("trading-securities source-repair column is invalid")
            seen_columns.add(column["column_ordinal"])
        material = canonical_clone_v1(repair)
        repair["repair_id"] = "gjtsafav1:repair:" + canonical_json_sha256_v1(material)
    ...
    compiled["overlay_id"] = "gjtsafav1:overlay:" + canonical_json_sha256_v1(compiled)
    return compiled


def compile_gemini_json_trading_securities_activity_family_specs_v1(
    topology_spec: Any,
    evaluation_spec: Any,
    schema_binding_spec: Any,
    source_repair_spec: Any | None = None,
) -> dict[str, Any]:
    """Compile Family 32 and its immutable source adapter."""
    ...
    if (
        ...
        raise _error("trading-securities declarative adapter boundary is invalid")
    raw = (
        _load_default_source_repair_artifact_v1()
        if source_repair_spec is None
        else source_repair_spec
    )
    compiled["trading_securities_activity_adapter_format_version"] = ADAPTER_FORMAT_VERSION
    compiled["trading_securities_activity_source_repair_overlay"] = (
        _compile_source_repair_artifact_v1(raw)
    )
    compiled["trading_securities_activity_source_repair_spec_sha256"] = (
        canonical_json_sha256_v1(raw)
    )
    return compiled


def build_gemini_json_trading_securities_activity_region_query_receipt_v1(
    regions: Any,
) -> dict[str, Any]:
...
def _region_table(
    ...
    try:
        ...
        raise _error("trading-securities region does not resolve one source table") from exc
    if type(page) is not dict or type(section) is not dict or type(table) is not dict:
        raise _error("trading-securities region source table is invalid")
...
def _apply_source_repairs_v1(
    *,
    ...
    receipts = []
    overlay = compiled_specs["trading_securities_activity_source_repair_overlay"]
    for repair in overlay["repairs"]:
        ...
        if any(
            ...
            raise _error("trading-securities source-repair locator drifted")
        ...
        if (
            ...
            raise _error("trading-securities source-repair base content drifted")
        ...
        for cell in repair["cell_repairs"]:
            ...
            try:
                ...
                raise _error("trading-securities source-repair cell locator drifted") from exc
            if (
                ...
                raise _error("trading-securities source-repair cell source drifted")
        ...
        for column_repair in repair["column_repairs"]:
            ...
            try:
                ...
                raise _error("trading-securities source-repair column locator drifted") from exc
            if (
                column.get("header_path_exact") != column_repair["header_path_exact"]
                or column.get("value_kind") != column_repair["original_value_kind"]
            ):
                raise _error("trading-securities source-repair column source drifted")
        ...
        receipts.append(
            {
                **material,
                "receipt_id": "gjtsafav1:source-repair:"
...
def _promote_visible_terminal_results_v1(
    *,
    ...
    for region in regions:
        ...
        if classification.get("family_root_row_ordinals"):
            continue
        declared_roles = {
            item.get("role")
        ...
        matches = [
            ...
            and _without_leading_ordinal(_normalized(row.get("label_exact")))
        ...
        material = {
            ...
            "row_ordinal": ordinal,
            "rule": (
                "UNIQUE_LAST_SOURCE_VISIBLE_TOTAL_OR_SUBTOTAL_LABELLED_TONG_OR_CONG_"
        ...
        receipts.append(
            {
                **material,
                "receipt_id": "gjtsafav1:terminal-result:"
                + canonical_json_sha256_v1(material),
            }
        )
    return receipts
    ...
    *,
    ...
    for region in regions:
        ...
        if _unit_axis(table, compiled_specs=compiled_specs).get("canonical_unit") != canonical_unit:
            raise _error("trading-securities corroborated unit token does not recompile")
        material = {
        ...
        receipts.append(
            {
                **material,
                "receipt_id": "gjtsafav1:unit:" + canonical_json_sha256_v1(material),
            }
        )
    return receipts
...
def _table_from_inventory(
    ...
    except (KeyError, GeminiJsonTradingSecuritiesActivityFamilyV1Error):
        return None
    ...
    *,
    ...
    material = {
        ...
        "rule": (
            ...
            "STATEMENT_PREFERRED_EXPLICIT_MILLION_VND_PRESENTATION_NO_VALUE_"
            "SELECTION_BLANK_COMPLETION_OR_SCALE_INFERENCE"
        ),
...
def _primary_query_recovery_v1(
    *,
    ...
    if type(inventory) is not list or _direct_note_candidate_axis(
    ...
    for item in inventory:
        ...
        if (
            ...
            continue
        candidates.append((region, receipt))
...
def adapt_gemini_json_trading_securities_activity_indexed_query_evidence_v1(
    value: Any,
    *,
        ...
        if recovered is not None:
            ...
            for item in cluster["declared_money_table_inventory"]:
                if (
                    ...
                    item["disposition"] = (
                        "SELECTED_PRIMARY_STATEMENT_EXACT_FAMILY_ROOT_AFTER_FAMILY32_RECEIPT"
                    )
...
def _bind_source_visible_family_result_v1(
    ...
    *,
    ...
    The shared F32 policy intentionally permits component-only notes.  Some
    ...
    if len(matches) != 1:
        raise _error("trading-securities visible result locator drifted")
    region = matches[0]
    ...
    if (
        ...
        raise _error("trading-securities visible result axes drifted after evaluation")
    row = rows[ordinal - 1]
    ...
    if type(values) is not list or any(column > len(values) for column in money_ordinals):
        raise _error("trading-securities visible result row drifted after evaluation")
    ...
    if component_roles:
        ...
        if exact_lane_ordinals:
            ...
            suffixes = product((-1, 1), repeat=len(component_roles))
            multiplier_candidates = [
                list(multipliers)
                for multipliers in suffixes
                if (
                    "INCOME_TRADING_SECURITIES" not in component_roles
                    or multipliers[component_roles.index("INCOME_TRADING_SECURITIES")] == 1
                )
                and all(
                    abs(
                        sum(
    ...
    receipt_material = {
        ...
        "rule": (
            "SOURCE_VISIBLE_RESULT_MAPS_DIRECTLY_COMPONENT_SIGN_ORIENTATION_"
            "IS_VETO_ONLY_ON_FULLY_OBSERVED_LANES_BLANK_LANES_EXCLUDED"
        ),
    }
    candidate["closure_receipt"]["source_visible_family_result_direct_mapping_receipt"] = {
        **receipt_material,
        "receipt_id": "gjtsafav1:root:" + canonical_json_sha256_v1(receipt_material),
    }
    ...
    candidate["closure_receipt"]["structural_root_receipt"]["mapping_policy"] = (
        "FAMILY32_SOURCE_VISIBLE_RESULT_DIRECT_COMPONENT_EQUATION_VETO"
    )
    ...
    *,
    ...
    material = {
        ...
        "source_repair_spec_sha256": compiled_specs[
            "trading_securities_activity_source_repair_spec_sha256"
        ],
    ...
    candidate["closure_receipt"]["trading_securities_activity_adapter_receipt"] = {
        **material,
        "adapter_receipt_id": "gjtsafav1:receipt:"
        + canonical_json_sha256_v1(material),
    }
...
def evaluate_gemini_json_trading_securities_activity_family_cluster_v1(
    *,
    regions: Any,
    ...
    """Evaluate Family 32 after exact, replayable source normalization."""

    if compiled_specs.get("topology", {}).get("family_id") != FAMILY_ID:
        raise _error("trading-securities adapter received another family")
    expected = build_gemini_json_trading_securities_activity_region_query_receipt_v1(regions)
    if type(query_receipt) is not dict or not same_typed_json_v1(query_receipt, expected):
        raise _error("trading-securities query receipt drifted")
    ...
    if len(region_axis) == 1:
        ...
        primary = _primary_statement_projection_v1(
            region=region_axis[0],
            pages=primary_source_pages,
            compiled_specs=compiled_specs,
        )
        if primary is not None:
            projected_pages, primary_receipt = primary
    ...
    if primary_receipt is not None:
        effective_specs["schema"]["root_mapping_policy"] = (
...
def validate_gemini_json_trading_securities_activity_family_candidate_replay_v1(
    value: Any,
    *,
    regions: Any,
    page_json_by_version: Mapping[str, dict[str, Any]],
    ...
    expected = evaluate_gemini_json_trading_securities_activity_family_cluster_v1(
        regions=regions,
    ...
    if type(value) is not dict or not same_typed_json_v1(value, expected):
        raise _error("trading-securities candidate replay drifted")
    return expected
