...
def _x_anchor(line: Mapping[str, Any], alignment: str) -> float:
    left, _top, right, _bottom = _bbox(line["bbox"])
    if alignment == "LEFT":
        return float(left)
    if alignment == "CENTER":
    ...
    if alignment == "RIGHT":
        return float(right)
    raise _error("numeric-column alignment mode drifted")


def _alignment_clusters(
    ...
    *,
    ...
    for line in sorted(candidates, key=lambda item: _x_anchor(item, alignment)):
        anchor = _x_anchor(line, alignment)
        target = next(
            (
                cluster
                for cluster in clusters
                if abs(anchor - float(median(_x_anchor(item, alignment) for item in cluster)))
                <= tolerance
            ),
            None,
        )
    ...
    return clusters if retain_singletons else repeated if repeated else clusters
...
def infer_numeric_column_centers_v1(
    ...
    *,
    ...
    """Infer numeric lanes under the most coherent body alignment.

    Financial values are commonly right-aligned, but some filings center or
    left-align them.  Clustering bbox centers alone creates a false extra lane
    whenever a short value such as ``17`` shares a right edge with longer
    values.  The generic rule therefore evaluates left, center and right
    anchors, chooses the axis with the fewest coherent clusters, then returns
    the median visual center of each selected cluster for downstream header
    projection.
    """
    ...
    if type(retain_singleton_columns) is not bool:
        raise _error("numeric singleton-column policy must be one exact bool")
    ...
    alternatives = [
        (
            ...
            _alignment_clusters(
                ...
                alignment=alignment,
                tolerance=tolerance,
                retain_singletons=retain_singleton_columns,
            ),
        )
        for alignment in ("CENTER", "RIGHT", "LEFT")
    ]
    _alignment, selected = min(
        alternatives,
        key=lambda item: (
            len(item[1]),
            -sum(len(cluster) for cluster in item[1]),
            ("CENTER", "RIGHT", "LEFT").index(item[0]),
        ),
    )
    centers = [float(median(_x_center(line) for line in cluster)) for cluster in selected]
...
def assign_value_row_lanes_v1(
    ...
    *,
    ...
    Missing lanes are not synthesized.  Retaining the ordinal is essential:
    a row with only one visible cell may contain the right/comparative lane,
    rather than the first/current lane.
    """
    ...
    return [
        {
            ...
            "column_ordinal": lane,
            "line": by_lane[lane][1],
            "row_affinity": round(by_lane[lane][0], 8),
        }
...
def assign_numeric_row_v1(
    ...
    *,
    ...
    """Compatibility projection of visible cells ordered by their lane."""

    return [
        item["line"]
        for item in assign_value_row_lanes_v1(
            lines,
...
def propose_missing_value_lane_regions_v1(
    lines: Sequence[Mapping[str, Any]],
    *,
    label_boxes: Sequence[Any],
    is_numeric: Callable[[Mapping[str, Any]], bool],
    ...
    The table body supplies the numeric lane centers and typical cell extents.
    A visible cell on the target row supplies the preferred row band; otherwise
    the label baseline and page-local numeric height are used.  Returned boxes
    are geometry proposals only and must be cropped from the authenticated page
    render and independently recognized before any blank/dash/number decision.
    """
    ...
    for box in numeric_boxes:
        ...
        domain_left, domain_right = domains[lane]
        if domain_left <= center <= domain_right:
            widths_by_lane[lane].append(box[2] - box[0])
    all_widths = [width for widths in widths_by_lane.values() for width in widths]
    fallback_width = float(median(all_widths)) if all_widths else scale * 4.0

    if visible:
        visible_boxes = [_bbox(item["line"]["bbox"]) for item in visible]
        row_center = float(median((box[1] + box[3]) / 2 for box in visible_boxes))
    ...
    numeric_row_center_clusters: list[list[float]] = []
    for center in sorted((box[1] + box[3]) / 2 for box in numeric_boxes):
        target = next(
            (
                cluster
                for cluster in numeric_row_center_clusters
                if abs(center - float(median(cluster))) <= scale * 0.55
            ),
            None,
        )
        if target is None:
            numeric_row_center_clusters.append([center])
        else:
            target.append(center)
    other_row_centers = [
        float(median(cluster))
        for cluster in numeric_row_center_clusters
        if abs(float(median(cluster)) - row_center) > scale * 0.55
    ]
    prior_centers = [center for center in other_row_centers if center < row_center]
    next_centers = [center for center in other_row_centers if center > row_center]
    if prior_centers:
        top = max(top, int(round((max(prior_centers) + row_center) / 2)))
    if next_centers:
        bottom = min(bottom, int(round((min(next_centers) + row_center) / 2)))
    if bottom <= top:
        raise _error("inferred missing-lane row band has no positive area")

    proposals = []
    for lane in missing_lanes:
        lane_widths = widths_by_lane[lane]
        typical_width = float(median(lane_widths)) if lane_widths else fallback_width
        domain_left, domain_right = domains[lane]
        half_width = max(typical_width * 0.7, scale * 1.5)
        left = max(0, int(round(max(domain_left, centers[lane] - half_width))))
        right = min(
            page_width,
            int(round(min(domain_right, centers[lane] + half_width))),
        )
        if right <= left:
            raise _error("inferred missing-lane column region has no positive area")
        proposals.append(
            {
                "column_center": centers[lane],
                "column_ordinal": lane,
                ...
                "raw_pixel_bbox": [left, top, right, bottom],
                "row_band_evidence": row_evidence,
                "visible_lane_ordinals": sorted(visible_lanes),
            }
        )
    return proposals
