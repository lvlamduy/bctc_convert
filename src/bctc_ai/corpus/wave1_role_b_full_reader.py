...
POLICY_RELATIVE_PATH = Path("config/corpus/bank-corpus-wave-1-role-b-full-reader-v1.yaml")
POLICY_SHA256 = "59667a43549e7ec936305b5733ea5d44275a7cd2ea01b91166d2f16f05402a48"
POLICY_SIZE_BYTES = 8_021
...
OUTPUT_RELATIVE_ROOT = Path("output/development/bank-corpus-wave-1-role-b-page-reader-v1/full-v1")
...
FULL_READER_IMPLEMENTATION_RELATIVE_PATHS = (
    ...
    Path("src/bctc_ai/corpus/wave1_role_b_full_reader.py"),
    ...
    Path("scripts/models/run_ppocrv6_wave1_full_worker.py"),
    Path("scripts/corpus/run_wave1_role_b_full_reader.py"),
)
...
def _load_policy(project_root: Path) -> dict[str, Any]:
    ...
    expected_root = {
        "version": 1,
        "policy": "BANK_CORPUS_WAVE_1_ROLE_B_FULL_PAGE_READER_V1",
...
def build_authenticated_control(project_root: Path, *, model_cache: Path) -> dict[str, Any]:
    ...
    control = {
        "format_version": "BANK_CORPUS_WAVE_1_ROLE_B_FULL_READER_CONTROL_V1",
...
def _consume_worker_response(
    ...
    *,
    ...
    if (
        ...
        or response.get("format_version") != "BANK_CORPUS_WAVE_1_PPOCRV6_FULL_WORKER_RESPONSE_V1"
            ...
            while True:
                ...
                for shard_id, process in sorted(processes.items()):
                    ...
                    if state["next_index"] < len(state["expected"]):
                        ...
                        try:
                            response_payload, response_stat = sentinel._hash_open_at(  # noqa: SLF001
                                response_fd, f"{request_sha}.response.json"
                            )
                        except FileNotFoundError:
                        ...
                        if response_payload is not None and (
                            stat.S_IMODE(response_stat.st_mode) != 0o444
                            or response_stat.st_nlink != 1
                        ):
                            raise WaveOneRoleBFullReaderError("worker response is not immutable")
