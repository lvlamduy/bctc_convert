...
- Latest full test run before the E-0011 documentation commit: 115 passed and 2 immutable-historical replays skipped; Ruff lint passed on the full source/test/script tree. E-0006 and E-0009 source/config identities remain verified, but exact replay is skipped when their historical algorithm byte hashes are not the current implementation. E-0010 and E-0011 provide current hash-locked integration regressions.
...
- A checkpointed PP-OCRv6 batch runner now loads the frozen detector/recognizer once per process and publishes one atomic page at a time. It rejects role relabeling, dirty upstream preprocessing, source/render/envelope drift, runtime/model/config/code drift, and mismatched orphan pages. A development smoke on TCB page 15 produced the same 50 lines/380 words and byte-identical OCR JSON SHA-256 `91779b3e22fadc01eeca7605c71a356e577e56541363ac91ea2750645721c54b` as E-0011; because the smoke used uncommitted code, it is mechanism evidence only.
