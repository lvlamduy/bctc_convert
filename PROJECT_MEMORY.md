...
- Latest full test run after structural fusion v2: 160 passed and 2 immutable-historical replays skipped; Ruff lint and the changed-file formatter gate passed. E-0006 and E-0009 source/config identities remain verified, but exact replay is skipped when their historical algorithm byte hashes are not the current implementation. E-0010 through E-0014 provide current hash-locked integration regressions; v2 has focused unit coverage pending formal E-0015 evidence.
...
- Fuse the now-sealed E-0014 Role B/Role C evidence without bank/page rules: parse every table block in reading order, infer optional index/label/note/value column roles, reconstruct Role C rows/axes, preserve cross-page continuation, and account explicitly for missing/extra rows. Values must not choose the structural alignment path.
