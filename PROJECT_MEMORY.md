...
- Latest full test run after statement-location v1 implementation: 143 passed and 2 immutable-historical replays skipped; Ruff lint passed on the full source/test/script tree and all five new/changed Python files pass the formatter gate. E-0006 and E-0009 source/config identities remain verified, but exact replay is skipped when their historical algorithm byte hashes are not the current implementation. E-0010, E-0011, and E-0012 provide current hash-locked integration regressions.
...
- Run the now-frozen Role B/Role C logic unchanged on MBB/VCB calibration sources, then on controlled distortions and an untouched holdout. Include rows with both period cells absent/missed, cross-page broken rows, dark headers, skew/perspective, quarterly/YTD axes, and consolidated scope.
- Rerun MBB/VCB statement location from the clean locator commit, hash-lock the two outputs as E-0013, and use only their eligible page contracts for higher-DPI Role B/Role C processing.
