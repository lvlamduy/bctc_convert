...
- Latest full test run: 99 passed and 1 immutable-historical replay skipped in 36.34 seconds; Ruff lint passed. The E-0006 source/config identity remains verified, but replay is skipped because its exact historical algorithm hashes are not the current implementation. E-0010 provides a current hash-locked integration regression, and bootstrap now verifies the tracked calibration/config/algorithm/local-seal identities.
...
- Add an independent word/cell-box OCR and targeted native-resolution rereads to the detected E-0010 page-14 row collapses and page-15 multi-number cell; measure recovery without string splitting or history repair.
