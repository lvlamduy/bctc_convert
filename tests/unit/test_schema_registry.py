...
def test_supplied_schema_is_imported_without_reordering(project_root):
    ...
    assert len(items) == len({item.schema_id for item in items}) == 1947
    ...
    assert tm[-1].display_order == 1712
...
def test_universal_schema_contract_is_base_plus_audited_additions(project_root):
    ...
    assert registry["universal_schema"]["revision"] == "UNIVERSAL_BANK_BCTC_SCHEMA@6068"
    assert registry["universal_schema"]["high_watermark"] == 6068
    assert registry["universal_schema"]["item_count"] == 1947
