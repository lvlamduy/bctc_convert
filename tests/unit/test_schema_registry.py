...
def test_supplied_schema_is_imported_without_reordering(project_root):
    ...
    assert len(items) == len({item.schema_id for item in items}) == 1955
    ...
    assert by_id[6073].canonical_name == "+ Thương mại, dịch vụ"
    assert by_id[6073].previous_id == 734
    assert by_id[6073].next_id == 735
    assert by_id[6074].canonical_name == "+ Hợp tác xã và công ty tư nhân"
    assert by_id[6074].previous_id == 776
    assert by_id[6074].next_id == 777
    assert by_id[6075].canonical_name == ("Thu nhập lãi cho vay khách hàng và các TCTD khác")
    assert by_id[6075].previous_id == 1145
    assert by_id[6075].next_id == 1146
    assert by_id[6076].canonical_name == "Thu phí nghiệp vụ thư tín dụng (L/C)"
    assert by_id[6076].previous_id == 1147
    assert by_id[6076].next_id == 1148
    ...
    assert tm[-1].display_order == 1720
...
def test_universal_schema_contract_is_base_plus_audited_additions(project_root):
    ...
    assert registry["universal_schema"]["revision"] == "UNIVERSAL_BANK_BCTC_SCHEMA@6076"
    assert registry["universal_schema"]["high_watermark"] == 6076
    assert registry["universal_schema"]["item_count"] == 1955
