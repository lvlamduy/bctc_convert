...
_PATH = _ROOT / "scripts/experiments/scan_central_bank_deposits_full_document_vietocr_v1.py"
_SPEC = importlib.util.spec_from_file_location(
    "scan_central_bank_deposits_full_document_vietocr_v1", _PATH
)
...
def _matcher_result(*, accepted: bool) -> dict[str, object]:
    return {
        ...
        "metrics": {
            "complete_central_bank_deposit_region_count": int(accepted),
            "near_region_count": 2,
        },
...
def test_build_scans_all_eight_documents_and_replay_rejects_tamper(
    ...
    class Matcher:
        ...
        @classmethod
        def build_central_bank_deposits_variant_graph_document_v1(cls, pages: object) -> object:
            del pages
            cls.calls += 1
            return _matcher_result(accepted=cls.calls in {2, 3, 8})
    ...
    result = scan.build_central_bank_deposits_full_document_scan_v1({})

    assert result["metrics"] == {
        "accepted_numeric_graph_count": 0,
        "central_bank_deposit_region_count": 3,
        "document_count": 8,
    ...
    with pytest.raises(scan.CentralBankDepositsFullDocumentScanV1Error):
        scan.validate_central_bank_deposits_full_document_scan_replay_v1(forged, {})
