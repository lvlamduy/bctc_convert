...
_PATH = _ROOT / "scripts/experiments/scan_other_assets_full_document_vietocr_v1.py"
_SPEC = importlib.util.spec_from_file_location("scan_other_assets_full_document_vietocr_v1", _PATH)
...
def test_scanner_is_bank_blind_and_covers_all_eight_documents(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _patch_axis(monkeypatch)
    result = scanner.build_other_assets_full_document_scan_v1({})
...
def test_scanner_exact_replay_rejects_coordinated_tamper(
    ...
    forged["scan_id"] = "oafdsv1:scan:" + scanner.canonical_json_sha256_v1(material)

    with pytest.raises(scanner.OtherAssetsFullDocumentScanV1Error, match="replay exactly"):
        scanner.validate_other_assets_full_document_scan_replay_v1(forged, {})
