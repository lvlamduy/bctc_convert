...
def _build_project(source_root: Path, project_root: Path) -> dict[str, Any]:
    ...
    reason_counts = {
        ...
        "UNASSESSED_OUTSIDE_BOUNDED_TABLE_SUBTREE": 1716,
    }
    ...
    mapping_payload = {
        ...
        "schema": {"statement_type": "TM", "item_count": 1719},
        ...
        "coverage": {
            "statement_type": "TM",
            "schema_item_count": 1719,
            "schema_disposition_count": 1719,
...
def test_policy_and_public_api_are_bank_agnostic(synthetic: dict[str, Any], policy) -> None:
    assert policy.schema_disposition_count == 1719
...
def test_workbook_is_complete_auditable_and_formula_free(
    ...
    assert artifacts.summary["tm_schema_disposition_count"] == 1719
    ...
    try:
        ...
        assert workbook["SCHEMA_DISPOSITIONS"].max_row == (
            artifacts.summary["tm_schema_disposition_count"] + 1
        )
...
def test_zero_accepted_root_and_mismatch_equation_are_valid_generic_cases(
    ...
    mapping["coverage"]["terminal_outcome_counts"] = {
        outcome: 1719 if outcome == "UNRESOLVED" else 0 for outcome in exporter._TERMINAL_OUTCOMES
    }
    mapping["coverage"]["reason_counts"] = {"UNRESOLVED_LOCAL_COMPLETENESS_OR_EQUATION": 1719}
...
def test_completed_pair_uses_old_producer_not_current_semantics(
    ...
    assert result.summary["tm_schema_disposition_count"] == 1719
...
def test_real_registered_vpb_projection_counts_and_sheets(project_root: Path) -> None:
    ...
    assert built.summary["tm_schema_disposition_count"] == 1714
