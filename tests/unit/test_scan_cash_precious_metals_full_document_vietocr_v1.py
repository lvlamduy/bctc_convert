...
_PATH = _ROOT / "scripts/experiments/scan_cash_precious_metals_full_document_vietocr_v1.py"
_SPEC = importlib.util.spec_from_file_location(
    "scan_cash_precious_metals_full_document_vietocr_v1", _PATH
)
...
def _matcher_result(*, accepted: bool) -> dict[str, object]:
    return {
        ...
        "metrics": {
            "complete_cash_precious_metals_region_count": int(accepted),
            "near_region_count": 1,
        },
...
def test_build_scans_all_eight_documents_and_replay_rejects_tamper(
    ...
    class Matcher:
        ...
        @classmethod
        def build_cash_precious_metals_variant_graph_document_v1(cls, pages: object) -> object:
            del pages
            cls.calls += 1
    ...
    result = scan.build_cash_precious_metals_full_document_scan_v1({})

    assert result["metrics"] == {
        "accepted_numeric_graph_count": 0,
        "cash_precious_metals_region_count": 3,
        "document_count": 8,
    ...
    with pytest.raises(scan.CashPreciousMetalsFullDocumentScanV1Error):
        scan.validate_cash_precious_metals_full_document_scan_replay_v1(forged, {})
