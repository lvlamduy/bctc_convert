...
from test_gemini_json_categorical_period_matrix_v1 import (
    _compiled,
    _json,
    _page,
    _table,
...
def _fixture(tmp_path):
    ...
    absent = _ingest(
        ...
        source_sha256="3" * 64,
        page_json=_page(None, title="Tài sản cố định"),
    )
...
def test_exchange_rate_query_sweep_and_sqlite_candidate_replay(tmp_path) -> None:
    ...
    sweep = build_gemini_json_flat_family_sweep_v1(
        ...
        topology_spec=_json("tm-exchange-rate-topology-v1.json"),
        evaluation_spec=_json("tm-exchange-rate-evaluation-v1.json"),
        schema_binding_spec=_json("tm-exchange-rate-schema-binding-v1.json"),
        trials=trials,
...
def test_exchange_rate_sqlite_replay_rejects_coherent_source_value_drift(tmp_path) -> None:
    database, selected, evidence, trials, compiled = _fixture(tmp_path)
    forged = copy.deepcopy(trials)
    candidate = forged[0]["candidates"][0]
    candidate["closure_receipt"]["resolved_rows"][0]["cells"][0]["source_text"] = "99.999"
    ...
    with pytest.raises(ValueError, match="normalization drifted|does not replay"):
        validate_selected_equity_matrix_family_candidate_replays_v1(
            database,
            selected_page_json_version_ids=selected,
            compiled_specs=compiled,
...
def test_exchange_rate_query_binding_rejects_candidate_deletion(tmp_path) -> None:
    _database, _selected, evidence, trials, _compiled_specs = _fixture(tmp_path)
    forged = copy.deepcopy(trials)
    forged[0].update(
        {
            "candidate_count": 0,
