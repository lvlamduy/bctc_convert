...
from bctc_ai.ocr import family_first_ppocrv6_numeric_sharded_runner_v2 as runner
...
def test_fixed_shard_ranges_cover_axis_once() -> None:
    ...
    with pytest.raises(runner.FamilyFirstPPocrV6NumericShardedRunnerV2Error):
...
def test_missing_orchestrator_reuses_only_valid_completed_shards(
    ...
    result = runner.run_authenticated_family_first_ppocrv6_numeric_missing_shards_v2(
        tmp_path,
        object(),
        model_cache=Path("model-cache"),
    )
...
def test_aggregate_requires_and_replays_complete_gap_free_shard_axis(
    ...
    aggregate = runner.aggregate_authenticated_family_first_ppocrv6_numeric_v2(
        tmp_path,
        object(),
        model_cache=Path("model-cache"),
    )
    replay, _manifest_payload, proposals, offsets = (
        runner.validate_authenticated_family_first_ppocrv6_numeric_aggregate_v2(
            tmp_path,
            object(),
            model_cache=Path("model-cache"),
        )
...
def test_numeric_v2_trust_paths_cover_every_static_local_import() -> None:
    closure = _local_import_closure(
        (
            "bctc_ai.evaluation.family_first_ppocrv6_numeric_index_v2",
            "bctc_ai.evaluation.family_first_semantic_label_archive_v1",
            "bctc_ai.ocr.family_first_ppocrv6_numeric_sharded_runner_v2",
