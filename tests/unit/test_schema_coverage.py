...
def test_every_consumer_contains_tm_1944_in_workbook_order(project_root):
    ...
    assert len(contract.targets) == 1955
...
def test_mandatory_search_requires_1944_from_both_independent_roles(project_root):
    ...
    assert complete.universal_target_count == 1955
    ...
    assert complete.completed_count_by_role == {"ROLE_A": 1955, "ROLE_B": 1955}
    assert complete.outcome_count_by_role["ROLE_A"]["NOT_OBSERVED"] == 1955
    assert complete.outcome_count_by_role["ROLE_B"]["NOT_OBSERVED"] == 1954
    assert complete.outcome_count_by_role["ROLE_B"]["OBSERVED_VALUE"] == 1
    assert complete.applicable_count_by_role == {"ROLE_A": 1955, "ROLE_B": 1955}
    assert complete.observed_count_by_role == {"ROLE_A": 0, "ROLE_B": 1}
    assert complete.mapped_numeric_count_by_role == {"ROLE_A": 0, "ROLE_B": 1}
    assert complete.not_observed_count_by_role == {"ROLE_A": 1955, "ROLE_B": 1954}
...
def test_mandatory_search_preserves_all_universal_terminal_outcomes(project_root):
    ...
    for role in contract.mandatory_search_roles:
        for outcome in canonical_outcomes:
            expected = 1948 if outcome == "NOT_OBSERVED" else 1
        ...
        assert result.not_observed_count_by_role[role] == 1948
        ...
        assert result.applicable_count_by_role[role] == 1954
