...
_PATH = (
    _ROOT / "scripts/experiments/build_annual_2025_interest_rate_risk_rotated_ppocrv6_panel_v1.py"
)
_SPEC = importlib.util.spec_from_file_location(
    "build_annual_2025_interest_rate_risk_rotated_ppocrv6_panel_v1_test_target", _PATH
)
...
def test_panel_keeps_all_graph_and_geometry_selected_pages_in_upright_space(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setattr(builder, "PROJECT_ROOT", tmp_path)
    monkeypatch.setattr(builder, "OUTPUT_ROOT", Path("output/interest-panel"))
    monkeypatch.setattr(builder, "MANIFEST_PATH", Path("output/interest-panel/panel_manifest.json"))
    monkeypatch.setattr(
        builder, "_interest_rate_risk_selection", lambda: copy.deepcopy(_selection())
    )
    core = builder._configured_core()
    monkeypatch.setattr(core, "_clean_git", lambda: {"commit": "a" * 40, "dirty": False})

    manifest = builder.build_annual_2025_interest_rate_risk_rotated_ppocrv6_panel_v1()
    _write_reader_output(tmp_path, manifest)
    projection = builder.read_verified_annual_2025_interest_rate_risk_rotated_ppocrv6_panel_v1()
...
def test_selection_contract_is_bank_and_page_blind() -> None:
    assert builder.SELECTION_RULE == (
        "UNIQUE_COMPLETE_INTEREST_RATE_RISK_REGION_AND_GEOMETRY_ROTATED_SOURCE_AXIS_TRUE"
    )
