...
_PATH = _ROOT / "scripts/experiments/scan_trading_securities_activity_full_document_vietocr_v1.py"
_SPEC = importlib.util.spec_from_file_location(
    "scan_trading_securities_activity_full_document_vietocr_v1", _PATH
)
...
def _page() -> dict[str, object]:
    texts = [
        "16. Lãi/(lỗ) thuần từ mua bán chứng khoán kinh doanh",
...
def test_scanner_covers_all_eight_documents(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    result = scanner.build_trading_securities_activity_full_document_scan_v1({})
...
def test_scanner_exact_replay_rejects_coordinated_tamper(
    ...
    forged["scan_id"] = "tsafdsv1:scan:" + scanner.canonical_json_sha256_v1(material)
    with pytest.raises(
        scanner.TradingSecuritiesActivityFullDocumentScanV1Error,
        match="replay exactly",
    ):
        scanner.validate_trading_securities_activity_full_document_scan_replay_v1(forged, {})
