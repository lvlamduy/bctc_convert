...
_PATH = _ROOT / "scripts/experiments/scan_interbank_deposits_loans_full_document_vietocr_v1.py"
_SPEC = importlib.util.spec_from_file_location(
    "scan_interbank_deposits_loans_full_document_vietocr_v1", _PATH
)
...
def _matcher_result(*, accepted: bool) -> dict[str, object]:
    return {
        ...
        "metrics": {
            "complete_interbank_deposit_loan_region_count": int(accepted),
            "near_region_count": 3,
        },
        "status": "ACCEPTED_UNIQUE_VARIANT_GRAPH" if accepted else "UNRESOLVED_NO_COMPLETE_REGION",
        "uniqueness": {
...
def test_build_scans_all_eight_documents_and_replay_rejects_tamper(
    ...
    class Matcher:
        ...
        @classmethod
        def build_interbank_deposits_loans_variant_graph_document_v1(cls, pages: object) -> object:
            del pages
            cls.calls += 1
            return _matcher_result(accepted=cls.calls in {1, 2, 3, 6, 7, 8})
    ...
    result = scan.build_interbank_deposits_loans_full_document_scan_v1({})

    assert result["metrics"] == {
        "accepted_numeric_graph_count": 0,
        "document_count": 8,
        "document_unique_structural_match_count": 6,
        "interbank_deposit_loan_region_count": 6,
    ...
    with pytest.raises(scan.InterbankDepositsLoansFullDocumentScanV1Error):
        scan.validate_interbank_deposits_loans_full_document_scan_replay_v1(forged, {})
