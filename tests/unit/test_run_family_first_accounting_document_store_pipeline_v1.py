...
def test_document_store_pipeline_builds_and_replays_without_ocr(
    ...
    replayed = subject.run_family_first_accounting_document_store_pipeline_v1(
        tmp_path, command="verify", jobs=1, **kwargs
    )
    assert replayed == built
    assert (tmp_path / "evidence.json").read_bytes() == evidence_bytes
    assert (tmp_path / "mapping.json").read_bytes() == mapping_bytes
    assert calls == {"authenticate": 2, "build": 2, "jobs": [1, 1]}


def test_document_store_pipeline_accepts_host_worker_ceiling(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    _patch_specs(monkeypatch)
    evidence = {
    ...
    mapping = {
        ...
        "family_id": "FAMILY",
        "mapping_id": "ffasmv1:mapping:" + "2" * 64,
        "metrics": {"verified_document_count": 140},
    }
    monkeypatch.setattr(
        subject.store_v1,
        "authenticate_family_first_document_evidence_store_v1",
    ...
    monkeypatch.setattr(
        ...
        lambda _cap, _family, _evaluation, *, jobs: (
            evidence if jobs == 16 else pytest.fail("worker ceiling was not forwarded")
        ),
    )
    monkeypatch.setattr(
        subject.mapping_v1,
    ...
    result = subject.run_family_first_accounting_document_store_pipeline_v1(
        tmp_path,
        family_spec_path=Path("family.json"),
        evaluation_spec_path=Path("evaluation.json"),
        schema_binding_spec_path=Path("schema.json"),
    ...
    assert result["document_trial_worker_count"] == 16
