...
_PATH = _ROOT / "scripts/experiments/scan_loan_geography_full_document_vietocr_v1.py"
_SPEC = importlib.util.spec_from_file_location(
    "scan_loan_geography_full_document_vietocr_v1", _PATH
)
...
def _matcher_result(*, accepted: bool, broad: bool) -> dict[str, object]:
    return {
        "format_version": scan.MATCHER_FORMAT,
        "metrics": {
            "complete_customer_loan_geography_region_count": int(accepted),
            "near_region_count": int(broad),
...
def test_build_scans_all_eight_documents_and_replay_rejects_tamper(
    ...
    class Matcher:
        ...
        @classmethod
        def build_loan_geography_variant_graph_document_v1(cls, pages: object) -> object:
            del pages
            cls.calls += 1
            return _matcher_result(accepted=cls.calls in {2, 8}, broad=cls.calls in {1, 3, 4})
    ...
    result = scan.build_loan_geography_full_document_scan_v1({})

    assert result["metrics"] == {
        "accepted_numeric_graph_count": 0,
        "broad_scope_near_region_count": 3,
        "customer_loan_geography_region_count": 2,
        "document_count": 8,
        "document_unique_structural_match_count": 2,
        "mapping_verified_count": 0,
        "near_region_count": 3,
        "segment_report_negative_control_count": 8,
        "unresolved_document_count": 6,
    }
    forged = copy.deepcopy(result)
    forged["trials"][0]["document_provenance"] = "MBB"
    with pytest.raises(scan.LoanGeographyFullDocumentScanV1Error):
        scan.validate_loan_geography_full_document_scan_replay_v1(forged, {})
